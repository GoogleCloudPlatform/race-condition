...
async def test_no_simulation_id_still_works(_patch_simulation_registry):
    """Events without simulation_id should not crash."""
    reg = _patch_simulation_registry
    ...
    # With the simulation_id fix, context_id is used as the simulation_id
    # when no explicit simulation_id exists (direct A2A call path).
    # register is called for both context_id and vertex session_id.
    assert reg.register.call_count == 2
    reg.register.assert_any_call("ctx-plain", "ctx-plain")
    reg.register.assert_any_call("vertex-no-sim", "ctx-plain")
