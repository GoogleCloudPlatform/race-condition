...
async def test_redis_dash_log_plugin_model_end_preamble():
    with (
        ...
        plugin = RedisDashLogPlugin()
        # Payload includes `response.content` so the gate in
        # `_emit_narrative` (clean_text + final_text) emits a text event.
        # Empty/content-less model_end payloads now correctly produce no
        # narrative -- see test_dash_log_text_emission.py.
        payload = {
            "type": "model_end",
            "agent": "orchestrator_agent",
            "response": {"content": "all clear"},
        }
...
async def test_redis_dash_log_plugin_display_name_fallback():
    ...
    with (
        ...
        # `response.content` required so the narrative gate emits -- see
        # test_redis_dash_log_plugin_model_end_preamble for context.
        payload = {
            "type": "model_end",
            "agent": "runner_agent",
            "response": {"content": "tick complete"},
        }
