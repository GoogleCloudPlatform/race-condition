...
async def test_execute_orchestration_event(executor, mock_runner):
    ...
    # Create a real SimulationNetworkPlugin instance so isinstance() works
    # without patching the builtin (which interferes with other isinstance
    # calls in the executor).
    from agents.utils.simulation_plugin import SimulationNetworkPlugin

    mock_plugin = MagicMock(spec=SimulationNetworkPlugin)
