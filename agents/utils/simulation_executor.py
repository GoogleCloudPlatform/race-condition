...
from agents.utils.redis_pool import get_shared_redis_client  # noqa: E402
...
class SimulationExecutor(AgentExecutor):
    """Custom A2A executor for simulation agents.

    Used in both Agent Engine (via deployment.py) and local serving (via
    a2a.py/register_a2a_routes).  Handles simulation ID mapping that the
    built-in A2aAgentExecutor does not: context_id → session_id bridging
    and simulation_id propagation so DashLogPlugin and the gateway Hub
    can route events to the correct frontend subscribers.
    ...
    - Accepts a pre-built Runner for local serving (avoids double init)
    """
    ...
    def __init__(
        ...
        # Accept a pre-built Runner (local serving path) or lazily
        # build one on first execute() (Agent Engine path).
        if runner is not None:
            self._runner: Optional[Runner] = runner
            self._session_manager: Optional[SessionManager] = SessionManager(
                session_service=runner.session_service,
                cache_maxsize=1000,
            )
            logger.info(
                "SimulationExecutor initialized with external runner: agent=%s, session=%s",
                self._agent_name,
                type(runner.session_service).__name__,
            )
        else:
            self._runner = None
    ...
    def _init_runner(self) -> None:
        ...
        assert self._agent_getter is not None, "agent_getter required for lazy init"
    ...
    async def execute(
        ...
        # DIAGNOSTIC: Log all IDs at entry for simulation ID mismatch debugging.
        # WARNING level so it appears in Cloud Logging on Agent Engine
        # (AE filters INFO from custom modules by default).
        msg_context_id = context.message.context_id if context.message else None
        logger.warning(
            "SIMULATION_EXECUTOR_IDS: agent=%s context.context_id=%s msg.context_id=%s "
            "task_id=%s context_id(resolved)=%s",
            self._agent_name,
            context.context_id,
            msg_context_id,
            task_id,
        ...
        try:
            ...
            elif isinstance(event, dict) and "simulation_id" in event:
                # Direct A2A call from planner: simulation_id is in the
                # message payload body.  On Agent Engine the A2A transport
                # assigns its own context_id, so the payload is the only
                # reliable source of the planner's simulation_id.
                _event_simulation_id = event.get("simulation_id")
                logger.warning(
                    "SIMULATION_EXECUTOR: Extracted simulation_id from payload: %s",
                    _event_simulation_id,
                )
                # No dispatch handling -- fall through to normal query path.

            # Orchestration dispatch (only for events with a "type" field).
            if isinstance(event, dict) and "type" in event:
            ...
            # Not a JSON event or type error in orchestration parsing,
            # proceed as normal user query.
            logger.warning(
                "SIMULATION_EXECUTOR: Orchestration parse error (%s): %s", type(_parse_err).__name__, _parse_err
            )

        # DEDUP GUARD: Prevent duplicate runner.run_async() invocations.
        #
        # Two dedup scopes (different agent classes need different keys):
        #
        # 1. Simulator-class agents: receive parallel planner→simulator
        #    submissions. On GCP the gateway dispatches to the planner twice
        #    (~3s apart); each planner copy then calls submit_plan_to_simulator
        #    with its own freshly-generated A2A task_id but the SAME context_id.
        #    The dedup key is `(agent, context_id)` so the second submission
        #    is blocked even though task_ids differ.
        #
        # 2. Planner-class agents (and everything else): receive user
        #    broadcasts via the gateway. Each user click on the A2UI surface
        #    (e.g. the "Run Simulation" button) is a NEW logical message with
        #    a fresh task_id but the SAME session/context_id. The dedup key
        #    is `(agent, context_id, task_id)` so legitimate follow-up clicks
        #    pass through while genuine gateway-replay duplicates (which carry
        #    the same task_id) are blocked.
        #
        # The lock TTL (600s) covers the maximum simulation duration.
        if self._agent_name.startswith("simulator"):
            _dedup_key = f"sim:exec_lock:{self._agent_name}:{context_id}"
        else:
            _dedup_key = f"sim:exec_lock:{self._agent_name}:{context_id}:{task_id}"
        try:
            _redis = get_shared_redis_client()
            if _redis is not None:
                _acquired = await _redis.set(_dedup_key, "1", nx=True, ex=600)
                if not _acquired:
                    logger.warning(
                        "DEDUP_BLOCK: duplicate execution blocked for %s context=%s task=%s key=%s",
                        self._agent_name,
                        context_id,
                        task_id,
                        _dedup_key,
                    ...
                    return
                logger.warning(
                    "DEDUP_ALLOW: acquired execution lock for %s context=%s task=%s key=%s",
                    self._agent_name,
                    context_id,
                    task_id,
                    _dedup_key,
            ...
            logger.warning("DEDUP_ERROR: Redis lock failed (%s), proceeding anyway", _dedup_err)
        ...
        try:
            ...
            #
            # Two sources of simulation_id, tried in order:
            # 1. Extracted from orchestration event payload (_event_simulation_id)
            # 2. Previously registered in Redis (lookup by context_id)
            #
            # Do NOT fall back to context_id -- that would cause every
            # request (including the initial user message) to register a
            # fake simulation_id, polluting DashLogPlugin events and
            # triggering the Hub's subscription filter to drop them.
            simulation_id = _event_simulation_id
            if not simulation_id:
                simulation_id = await simulation_registry.lookup(context_id)
            if simulation_id:
                # Register simulation_id for the Vertex session so
                # DashLogPlugin can resolve it, and also for context_id so
                # downstream sub-sessions (simulation_pipeline, tick, etc.)
                # can look it up.
                await simulation_registry.register(context_id, simulation_id)
                if session_id != context_id:
                    await simulation_registry.register(session_id, simulation_id)
                logger.warning(
                    "SIMULATION_EXECUTOR: Bridged simreg context=%s vertex=%s sim=%s",
                    context_id,
                    session_id,
                    simulation_id,
                )
