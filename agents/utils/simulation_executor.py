...
class SimulationExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        # DEDUP GUARD: Prevent duplicate runner.run_async() invocations for
        # the same agent + context_id.  On GCP, the gateway dispatches to the
        # planner twice (~3s apart), each planner calls submit_plan_to_simulator,
        # and the simulator receives two A2A message/send calls with the same
        # context_id.  The first call acquires a Redis NX lock; the second sees
        # the lock and exits immediately.  The lock TTL (600s) covers the
        ...
        try:
            ...
            if _redis is not None:
                ...
                if not _acquired:
                    logger.warning(
                        "DEDUP_BLOCK: duplicate execution blocked for %s context=%s",
                        self._agent_name,
                        context_id,
                    )
                    await updater.complete()
                    return
                logger.warning(
                    "DEDUP_ALLOW: acquired execution lock for %s context=%s",
                    self._agent_name,
                    context_id,
                )
