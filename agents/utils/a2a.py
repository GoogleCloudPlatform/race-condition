...
def prepare_simulation_agent(
    ...
    # Override version with BUILD_FINGERPRINT so post-deploy verification
    # can confirm the new code is actually serving traffic.
    build_fingerprint = os.environ.get("BUILD_FINGERPRINT")
    if build_fingerprint:
        agent_card.version = build_fingerprint
...
def register_a2a_routes(
    ...
    # Use SimulationExecutor instead of ADK's built-in A2aAgentExecutor.
    # SimulationExecutor maps context_id → session_id and registers
    # simulation_id in the simulation registry, which is required for
    # DashLogPlugin to emit events that the gateway Hub can route to
    # the correct frontend subscribers.
    agent_executor = SimulationExecutor(
        agent_name=app_name,
