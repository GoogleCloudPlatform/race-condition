...
class TestCreateServicesAgentEngine:
    """When AGENT_ENGINE_ID is set, create_services returns Agent Engine services."""

    @patch.dict(os.environ, {"AGENT_ENGINE_ID": "projects/123/locations/us/agents/abc"}, clear=True)
    def test_agent_engine_uses_vertex_session(self):
        """With AGENT_ENGINE_ID, session_service is VertexAiSessionService."""
        from agents.utils.runtime import create_services
    ...
    def test_agent_engine_target(self):
        """With AGENT_ENGINE_ID, target is 'agent_engine'."""
        from agents.utils.runtime import create_services
...
class TestCreateServicesAgentEnginePlatformEnvVar:
    ...
    @patch.dict(
        os.environ,
        {
            "AGENT_ENGINE_ID": "projects/123/locations/us/agents/abc",
            "GOOGLE_CLOUD_AGENT_ENGINE_ID": "99999",
        },
        clear=True,
    )
    def test_explicit_env_var_takes_precedence(self):
        """AGENT_ENGINE_ID takes precedence over GOOGLE_CLOUD_AGENT_ENGINE_ID."""
        from agents.utils.runtime import create_services

        config = create_services()
        assert type(config.session_service).__name__ == "VertexAiSessionService"
        # Verify the explicit ID is used, not the platform one
        assert config.session_service._agent_engine_id == "projects/123/locations/us/agents/abc"
...
class TestCreateServicesCloudRun:
    """When DATABASE_URL is set (no AGENT_ENGINE_ID), create_services returns Cloud Run services."""
...
class TestCreateServicesPrecedence:
    """AGENT_ENGINE_ID takes precedence over DATABASE_URL when both are set."""

    @patch.dict(
        os.environ,
        {"AGENT_ENGINE_ID": "projects/123/locations/us/agents/abc", "DATABASE_URL": "postgresql://localhost/test"},
        clear=True,
    )
    def test_agent_engine_takes_precedence(self):
        """When both AGENT_ENGINE_ID and DATABASE_URL are set, Agent Engine wins."""
        from agents.utils.runtime import create_services
