# Copyright 2026 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""Verify planner prompts instruct the LLM to use user-requested runner count."""

from agents.planner_with_eval.prompts import EXTENDED_SYSTEM_INSTRUCTION
from agents.planner_with_memory.prompts import MEMORY_SYSTEM_INSTRUCTION


def test_planner_with_eval_does_not_hardcode_runner_count():
    """The planner_with_eval prompt must not hardcode a specific runner_count value."""
    assert '"runner_count": 10000' not in EXTENDED_SYSTEM_INSTRUCTION
    assert '"runner_count": 10' not in EXTENDED_SYSTEM_INSTRUCTION
    assert "runner_count" in EXTENDED_SYSTEM_INSTRUCTION  # Still mentioned


def test_planner_with_eval_instructs_user_runner_count():
    """The planner_with_eval prompt must instruct the LLM to use the user's count."""
    lower = EXTENDED_SYSTEM_INSTRUCTION.lower()
    # Must reference both runner_count and user's preference in same prompt
    assert "runner_count" in EXTENDED_SYSTEM_INSTRUCTION
    assert "runner" in lower and "user" in lower, "Prompt must reference user's runner count preference"


def test_planner_with_memory_does_not_hardcode_runner_count():
    """The planner_with_memory prompt must not hardcode a specific runner_count value."""
    assert '"runner_count": 10' not in MEMORY_SYSTEM_INSTRUCTION
    assert "runner_count" in MEMORY_SYSTEM_INSTRUCTION  # Still mentioned


def test_planner_with_memory_instructs_user_runner_count():
    """The planner_with_memory prompt must instruct the LLM to use the user's count."""
    lower = MEMORY_SYSTEM_INSTRUCTION.lower()
    # Must reference both runner_count and user's preference in same prompt
    assert "runner_count" in MEMORY_SYSTEM_INSTRUCTION
    assert "runner" in lower and "user" in lower, "Prompt must reference user's runner count preference"


def test_planner_with_eval_mentions_runner_cap():
    """The planner_with_eval prompt must mention the runner cap as a max limit."""
    # Must mention "max 1,000" or "max 1000" — not just "1000" as a substring of "10000"
    lower = EXTENDED_SYSTEM_INSTRUCTION.lower()
    assert "max 1,000" in lower or "max 1000" in lower or "capped at 1,000" in lower


def test_planner_with_memory_mentions_runner_cap():
    """The planner_with_memory prompt must mention the runner cap as a max limit."""
    lower = MEMORY_SYSTEM_INSTRUCTION.lower()
    assert "max 1,000" in lower or "max 1000" in lower or "capped at 1,000" in lower
