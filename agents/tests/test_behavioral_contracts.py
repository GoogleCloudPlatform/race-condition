...
class TestMemoryContracts:
    ...
    def test_workflow_has_compliance_check(self):
        """Workflow must include get_local_laws_and_regulations."""
        workflow = PLANNER_WITH_MEMORY.sections["workflow"]
        assert "get_local_laws_and_regulations" in workflow

    # --- Memory-specific: storage ---

    def test_execution_has_record_simulation(self):
        """Execution must include record_simulation for memory storage."""
        execution = PLANNER_WITH_MEMORY.sections["execution"]
        assert "record_simulation" in execution

    def test_execution_has_store_simulation_summary(self):
        """Execution must include store_simulation_summary for future recall."""
        execution = PLANNER_WITH_MEMORY.sections["execution"]
