...
_build_waypoints_from_petals = _mod._build_waypoints_from_petals
_generate_spine_and_sprout = _mod._generate_spine_and_sprout
_split_route_by_road = _mod._split_route_by_road
_STRIP_HUB = _mod._STRIP_HUB
...
def planned_route(network: dict) -> dict:
    """Generate a real route using the planner's internal functions."""
    adj, landmarks, road_names, strip_nodes = _build_graph(network)
    nodes = set(adj.keys())

    waypoints = _build_waypoints_from_petals(
        ["west-flamingo-jones", "east-desertinn-maryland"],
        start=_STRIP_HUB,
    )

    route_coords, total_dist = _generate_spine_and_sprout(
        adj,
        nodes,
        landmarks,
...
class TestTrafficIntegration:
    ...
    def test_closure_analysis_finds_closures(self, planned_route: dict, network: dict) -> None:
        """identify_closed_segments finds route closures including Las Vegas Blvd.

        Route-coincident segments (like Las Vegas Boulevard, which the
        marathon runs along) appear in ``route_closures`` rather than
        ``closed``.  The ``closed`` list only contains collateral closures.
        """
        ...
        assert "Las Vegas Boulevard" in route_closure_names, (
            f"Las Vegas Boulevard should be a route closure; got {route_closure_names}"
        )
        # Route lines must not appear in closed
        closed_names = {seg.get("properties", {}).get("name", "") for seg in result["closed"]}
        assert "Las Vegas Boulevard" not in closed_names, (
            "Las Vegas Boulevard should not be in closed (it is a route line)"
        )
