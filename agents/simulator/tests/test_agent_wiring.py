...
class TestRaceEngineCallback:
    ...
    def _reset_race_engine(self):
        """Reset the module-level race_engine singleton before and after each test.

        Other test files (e.g. test_e2e_simulation) import and mutate the
        singleton, so we must restore the constructor default (200) both
        before and after to isolate these tests from cross-file pollution.
        """
        from agents.simulator.agent import race_engine

        race_engine.max_iterations = 200
        yield
        race_engine.max_iterations = 200
...
class TestModelOptimization:
    ...
    def test_root_uses_lite_model(self):
        ...
        assert root_agent.model.model == "gemini-flash-lite-latest"

    def test_pre_race_uses_lite_model(self):
        from agents.simulator.agent import pre_race_agent
        from agents.utils.global_gemini import GlobalGemini

        assert isinstance(pre_race_agent.model, GlobalGemini)
        assert pre_race_agent.model.model == "gemini-flash-lite-latest"

    def test_post_race_uses_lite_model(self):
        from agents.simulator.agent import post_race_agent
        from agents.utils.global_gemini import GlobalGemini

        assert isinstance(post_race_agent.model, GlobalGemini)
        assert post_race_agent.model.model == "gemini-flash-lite-latest"

    def test_root_has_sufficient_output_tokens(self):
        """Root agent needs enough tokens to relay simulation results.
        Budget raised from 256 to 1024 to prevent the ADK while-True
        loop from re-invoking the pipeline when the model can't fit
        a text response in too few tokens."""
        from agents.simulator.agent import root_agent
...
class TestPipelineReexecutionGuard:
    """The root agent must block simulation_pipeline from running twice."""

    def test_root_agent_has_before_tool_callback(self):
        """Root agent must have a before_tool_callback to guard re-execution."""
        from agents.simulator.agent import root_agent

        callbacks = root_agent.canonical_before_tool_callbacks
        assert len(callbacks) > 0, "Root agent must have a before_tool_callback"

    def test_guard_allows_first_pipeline_call(self):
        """First call to simulation_pipeline should be allowed (return None)."""
        from agents.simulator.agent import _guard_pipeline_reexecution
        from unittest.mock import MagicMock

        tool = MagicMock()
        tool.name = "simulation_pipeline"
        tool_context = MagicMock()
        tool_context.state = {}

        result = _guard_pipeline_reexecution(tool=tool, args={}, tool_context=tool_context)
        assert result is None

    def test_guard_blocks_second_pipeline_call(self):
        """Second call should return a dict (skip tool, feed result to LLM)."""
        from agents.simulator.agent import _guard_pipeline_reexecution
        from unittest.mock import MagicMock
        ...
        tool.name = "simulation_pipeline"
        tool_context = MagicMock()
        tool_context.state = {"_pipeline_executed": True}

        result = _guard_pipeline_reexecution(tool=tool, args={}, tool_context=tool_context)
        assert isinstance(result, dict)
        assert result["status"] == "skipped"

    def test_guard_ignores_other_tools(self):
        """Guard should not interfere with verify_plan or other tools."""
        from agents.simulator.agent import _guard_pipeline_reexecution
        from unittest.mock import MagicMock

        tool = MagicMock()
        tool.name = "verify_plan"
        tool_context = MagicMock()
        tool_context.state = {"_pipeline_executed": True}
    ...
    def test_guard_sets_flag_after_allowing(self):
        """After allowing execution, the flag must be set so next call is blocked."""
        from agents.simulator.agent import _guard_pipeline_reexecution
        from unittest.mock import MagicMock
        ...
        _guard_pipeline_reexecution(tool=tool, args={}, tool_context=tool_context)
        assert tool_context.state.get("_pipeline_executed") is True
