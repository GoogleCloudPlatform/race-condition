...
class TestPrepareSimulation:
    ...
    @pytest.mark.asyncio
    async def test_default_runner_count_is_10(self):
        """Default runner_count must be 10 to keep local dev simulations fast."""
        ...
        assert ctx.state["runner_count"] == 10
        assert ctx.state["simulation_config"]["runner_count"] == 10

    @pytest.mark.asyncio
    async def test_reads_simulation_config_from_plan(self):
    ...
    @pytest.mark.asyncio
    async def test_runner_type_always_runner_autopilot(self):
        """runner_type is always 'runner_autopilot' regardless of config."""
        plan = {
            "action": "execute",
            "narrative": "Autopilot test",
            "route": {"type": "FeatureCollection"},
            "simulation_config": {"runner_type": "some_other_runner"},
        }
        ctx = _make_tool_context()
    ...
    @pytest.mark.asyncio
    async def test_runner_type_ignores_config_and_env_var(self):
        """runner_type is hardcoded to runner_autopilot; config and env are ignored."""
        plan = {
            "action": "execute",
            "narrative": "Test",
            "route": {},
            "simulation_config": {"runner_type": "custom_runner_type"},
        }
        ctx = _make_tool_context()
    ...
    @pytest.mark.asyncio
    async def test_sets_simulation_id_from_session_id(self):
        """prepare_simulation should fall back to session.id when no simulation_id in state."""
        plan = {"action": "execute", "narrative": "Test", "route": {}}
        ctx = _make_tool_context()
        ctx.session.id = "sim-abc-123"

        result = await prepare_simulation(plan_json=json.dumps(plan), tool_context=ctx)

        assert result["status"] == "success"
        assert ctx.state["simulation_id"] == "sim-abc-123"
        assert result["simulation_id"] == "sim-abc-123"
    ...
    @pytest.mark.asyncio
    async def test_prepare_simulation_falls_back_to_session_id(self):
        """When state has NO simulation_id, prepare_simulation falls back
        to session.id (backward compat for direct invocation)."""
        plan = {"action": "execute", "narrative": "Test", "route": {}}
        ctx = _make_tool_context()
        ctx.session.id = "direct-invoke-session"
        # No simulation_id in state

        result = await prepare_simulation(plan_json=json.dumps(plan), tool_context=ctx)

        assert result["status"] == "success"
        assert ctx.state["simulation_id"] == "direct-invoke-session"
        assert result["simulation_id"] == "direct-invoke-session"

    @pytest.mark.asyncio
    async def test_runner_count_capped_at_max(self):
        """runner_count above MAX_RUNNER_COUNT is clamped."""
        plan = json.dumps(
            {
                "action": "execute",
                "simulation_config": {"runner_count": 5000},
            }
        )
        ctx = _make_tool_context()

        result = await prepare_simulation(plan, ctx)

        assert result["status"] == "success"
        assert result["runner_count"] == 1000
        assert result["capped_from"] == 5000
        assert ctx.state["runner_count"] == 1000
        assert ctx.state["simulation_config"]["runner_count"] == 1000

    @pytest.mark.asyncio
    async def test_runner_count_at_max_not_capped(self):
        """runner_count exactly at MAX_RUNNER_COUNT is not capped."""
        plan = json.dumps(
            {
                "action": "execute",
    ...
    @pytest.mark.asyncio
    async def test_runner_count_below_max_not_capped(self):
        """runner_count below MAX_RUNNER_COUNT passes through unchanged."""
        plan = json.dumps(
            {
                "action": "execute",
