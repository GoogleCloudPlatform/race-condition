...
class TestTickEventCollectorKey:
    ...
    def test_tick_event_omits_key_when_empty(self):
        """Backward compat: no key when collector_buffer_key not provided."""
        event = build_tick_event(tick=5, max_ticks=6, total_race_hours=6.0)
        assert "collector_buffer_key" not in event.data
...
class TestProcessTickDirectWrite:
    ...
    async def test_process_tick_rpushes_to_collector_buffer(self):
        ...
        from agents.npc.runner_shared.running import process_tick
        ...
        with patch(
            "agents.npc.runner_shared.running.get_shared_redis_client",
