...
RULES:
1. Call exactly ONE tool per request. Pass raw JSON through unchanged.
2. After a tool returns its result, respond with a SHORT text summary.
   Do NOT call any tool again. Your job is done — just reply with text.
3. NEVER call simulation_pipeline more than once.
"""
...
Valid runner types (in simulation_config.runner_type):
  runner_autopilot (default, deterministic), runner (LLM-powered, local),
  runner_cloudrun (LLM-powered, Cloud Run), runner_gke (LLM-powered, GKE).
  The runner_type from simulation_config is passed through to spawn_runners.
"""
...
pre_race_agent = LlmAgent(
    ...
    model=resilient_model("gemini-flash-lite-latest"),
...
def _guard_pipeline_reexecution(tool, args, tool_context):
    """Prevent simulation_pipeline from running more than once per turn.

    On Agent Engine, the ADK's while-True LLM loop can re-invoke the
    pipeline when the root model (gemini-flash-lite, low output tokens)
    fails to produce a text response after seeing the large pipeline
    result.  This callback gates the tool: first call is allowed and
    sets a flag; subsequent calls return a dict that the LLM sees as
    the tool result, giving it another chance to produce text.
    """
    if tool.name != "simulation_pipeline":
        return None

    if tool_context.state.get("_pipeline_executed"):
        logger.warning(
            "GUARD_BLOCK: simulation_pipeline already executed, blocking re-invocation. session_id=%s",
            getattr(tool_context, "session", {}) and str(getattr(tool_context.session, "id", "?")),
        )
        return {
            "status": "skipped",
            "reason": "Simulation pipeline already completed for this session. "
            "Summarize the results from the previous run.",
        }

    logger.warning(
        "GUARD_ALLOW: first simulation_pipeline call, allowing. session_id=%s",
        getattr(tool_context, "session", {}) and str(getattr(tool_context.session, "id", "?")),
    )
...
def get_agent(
    ...
    return LlmAgent(
        ...
        before_tool_callback=_guard_pipeline_reexecution,
    )
