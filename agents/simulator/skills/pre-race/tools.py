...
async def spawn_runners(count: int, tool_context: ToolContext) -> dict:
    ...
    # OIDC for OSS Cloud Run IAM mode: the gateway has roles/run.invoker
    # enforced via the cloud-run-iam module, so unauthenticated POSTs to
    # /api/v1/spawn would 401. resolve_audience picks IAP_CLIENT_ID in
    # dev/prod or the gateway URL in OSS; get_id_token returning None
    # (local dev / no ADC) means we send no header and rely on the
    # local gateway accepting unauth requests.
