...
async def prepare_simulation(
    ...
    if runner_count > MAX_RUNNER_COUNT:
        logger.warning(
            "prepare_simulation: runner_count %d exceeds MAX_RUNNER_COUNT %d, capping",
            runner_count,
            MAX_RUNNER_COUNT,
        )
        runner_count = MAX_RUNNER_COUNT
    ...
    # Use the simulation_id already captured by the root agent's callback.
    # Fall back to session.id for backward compatibility (direct invocation).
    if "simulation_id" not in tool_context.state:
        tool_context.state["simulation_id"] = str(tool_context.session.id)
    ...
    result = {
        ...
        "duration_seconds": duration_seconds,
        "tick_interval_seconds": tick_interval_seconds,
        "simulation_id": tool_context.state["simulation_id"],
    }
    if original_runner_count > MAX_RUNNER_COUNT:
        result["capped_from"] = original_runner_count
        result["message"] = (
            f"Simulation configured: {max_ticks} ticks at {tick_interval_seconds}s intervals, "
            f"{runner_count} runners (capped from {original_runner_count}; system maximum is {MAX_RUNNER_COUNT})."
        )
    return result
...
async def spawn_runners(count: int, tool_context: ToolContext) -> dict:
    ...
    runner_type = tool_context.state.get("runner_type", "runner_autopilot")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(spawn_url, json=payload) as resp:
                if resp.status != 200:
        ...
        # the simulator's MAX_RUNNER_COUNT, so the actual spawned count
        # can be less than requested.  Downstream tools (compile_results)
        # use state["runner_count"] for DNF calculation, so it MUST
        # reflect reality.
