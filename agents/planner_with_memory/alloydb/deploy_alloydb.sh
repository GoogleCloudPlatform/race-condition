...
#   3. Seeds the regulations table from seed_regulations.sql.
...
# ---------------------------------------------------------------------------
log "Applying schema (regulations + planned_routes + simulation_records)..."
psql "host=$ALLOYDB_HOST port=$ALLOYDB_PORT user=$ALLOYDB_USER dbname=$ALLOYDB_DATABASE" \
...
# ---------------------------------------------------------------------------
# Step 5: Seed regulations
# ---------------------------------------------------------------------------
log "Seeding regulations table..."
psql "host=$ALLOYDB_HOST port=$ALLOYDB_PORT user=$ALLOYDB_USER dbname=$ALLOYDB_DATABASE" \
  --file="$SCRIPT_DIR/seed_regulations.sql" \
  --quiet
REGULATION_COUNT="$(
  psql "host=$ALLOYDB_HOST port=$ALLOYDB_PORT user=$ALLOYDB_USER dbname=$ALLOYDB_DATABASE" \
    --tuples-only --command="SELECT COUNT(*) FROM regulations;" | xargs
)"
ok "Regulations table has $REGULATION_COUNT row(s)."
...
echo "  Regulations: $REGULATION_COUNT chunk(s)"
