...
_SCHEMA_LOCAL_SQL = _ALLOYDB_DIR / "schema_local.sql"
_SEED_LOCAL_SQL = _ALLOYDB_DIR / "seed_local.sql"
...
class TestSchemaLocalSql:
    """schema_local.sql must mirror production tables without AlloyDB extensions."""

    def test_file_exists(self):
        assert _SCHEMA_LOCAL_SQL.exists(), "schema_local.sql missing from alloydb/"

    def test_defines_all_four_tables(self):
        """Local schema must define all 4 tables from the production schema."""
        sql = _SCHEMA_LOCAL_SQL.read_text()
        for table in (
            "rules",
            "planned_routes",
            "simulation_records",
            ...
            assert table in sql, f"Table '{table}' missing from schema_local.sql"

    def test_simulation_summaries_has_vector_column(self):
        """simulation_summaries must include a VECTOR embedding column."""
        sql = _SCHEMA_LOCAL_SQL.read_text()
        # Find the simulation_summaries CREATE block and check for VECTOR
        summaries_idx = sql.find("simulation_summaries")
        assert summaries_idx != -1, "simulation_summaries not found"
        tail = sql[summaries_idx:]
        assert "VECTOR" in tail.upper(), "simulation_summaries missing VECTOR column"

    def test_no_alloydb_extensions(self):
        """Local schema must NOT contain AlloyDB-only SQL statements."""
        sql = _SCHEMA_LOCAL_SQL.read_text()
        # Strip SQL comments so we only check executable statements
        non_comment_lines = [line for line in sql.splitlines() if not line.strip().startswith("--")]
        executable_sql = "\n".join(non_comment_lines)
        assert "google_ml_integration" not in executable_sql, (
            "schema_local.sql must not use google_ml_integration extension"
        )
        assert "ai.initialize_embeddings" not in executable_sql, (
            "schema_local.sql must not call ai.initialize_embeddings"
        )
...
# ---------------------------------------------------------------------------
# 4. seed_local.sql quality tests
# ---------------------------------------------------------------------------


class TestSeedLocalSql:
    """seed_local.sql must contain idempotent regulation and route INSERTs."""

    def test_file_exists(self):
        assert _SEED_LOCAL_SQL.exists(), "seed_local.sql missing from alloydb/"

    def test_contains_regulation_inserts(self):
        """Must contain at least 3 LEGISLATION.txt chunk INSERTs."""
        sql = _SEED_LOCAL_SQL.read_text()
        matches = re.findall(r"LEGISLATION\.txt", sql)
        assert len(matches) >= 3, f"Expected >= 3 LEGISLATION.txt chunks, found {len(matches)}"

    def test_contains_all_four_route_ids(self):
        """Must contain all 4 seed route IDs."""
        sql = _SEED_LOCAL_SQL.read_text()
        for route_id in ("seed-0001", "seed-0002", "seed-0003", "seed-0004"):
            assert route_id in sql, f"Route ID '{route_id}' missing from seed_local.sql"

    def test_is_idempotent(self):
        """Must use ON CONFLICT for safe re-application."""
        sql = _SEED_LOCAL_SQL.read_text()
        assert "ON CONFLICT" in sql.upper()

    def test_targets_planned_routes_table(self):
        """Must INSERT into planned_routes."""
        sql = _SEED_LOCAL_SQL.read_text()
        assert "planned_routes" in sql


# ---------------------------------------------------------------------------
# 5. docker-compose.yml mount tests
# ---------------------------------------------------------------------------


class TestDockerCompose:
    """docker-compose.yml must mount both schema and seed SQL files."""

    def test_mounts_schema_sql(self):
        content = _DOCKER_COMPOSE_YML.read_text()
        assert "01_schema.sql" in content, "docker-compose.yml missing 01_schema.sql mount"

    def test_mounts_seed_sql(self):
        content = _DOCKER_COMPOSE_YML.read_text()
        assert "02_seed.sql" in content, "docker-compose.yml missing 02_seed.sql mount"
