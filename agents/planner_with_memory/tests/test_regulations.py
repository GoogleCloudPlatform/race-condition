...
_SEED_REGULATIONS_SQL = _ALLOYDB_DIR / "seed_regulations.sql"
...
class TestSeedRegulationsSQL:
    ...
    def test_targets_regulations_table(self):
        sql = _SEED_REGULATIONS_SQL.read_text()
        assert "regulations" in sql
...
class TestSchemaSql:
    ...
    def test_defines_regulations_table(self):
...
class TestSeedRoutesScript:
    ...
    @pytest.mark.asyncio
    async def test_skips_existing_rows(self):
        """ON CONFLICT path: execute returns 'INSERT 0 0' → not counted."""
        from agents.planner_with_memory.alloydb import seed_routes as seed_module

        mock_conn = AsyncMock()
        mock_conn.execute = AsyncMock(return_value="INSERT 0 0")  # conflict → 0 inserted
        mock_conn.close = AsyncMock()
...
class TestGetToolsInterface:
    ...
    The 6th memory tool must be get_local_laws_and_regulations.
    """
    ...
    def test_get_memory_tools_returns_eight_tools(self):
        """get_memory_tools() must return exactly 8 FunctionTools."""
    ...
    def test_get_memory_tools_names(self):
        """All 8 expected tool names must be present."""
        from agents.planner_with_memory.memory.adk_tools import get_memory_tools
        ...
        expected = {
            ...
            "get_route",
            "get_best_route",
            "get_local_laws_and_regulations",
