...
def test_root_agent_has_compliance_tool():
    """root_agent must have get_local_laws_and_regulations registered."""
    from agents.planner_with_memory.agent import root_agent

    tool_names = {getattr(t, "name", type(t).__name__) for t in root_agent.tools}
    assert "get_local_laws_and_regulations" in tool_names, "get_local_laws_and_regulations missing from agent tools"
