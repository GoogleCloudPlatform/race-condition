...
3. After sim completes, `record_simulation` AND `store_simulation_summary`
   in the SAME response.
4. Do NOT call report_marathon_route for pre-seeded routes (frontend has data).
5. Compose and emit the Simulation Results A2UI card via `validate_and_emit_a2ui`
   using surfaceId `"sim_results"`. Include the `route_id` in the simulation metadata.
6. STOP and wait for user feedback. The simulation is COMPLETE.
   Do NOT run another simulation. Do NOT restart the workflow.
   Do NOT call start_simulation or submit_plan_to_simulator again.
...
2. `start_simulation` AND `submit_plan_to_simulator(action="execute")` in SAME response.
   Use the user's runner_count (max 1,000); default to 10 if unspecified.
   If the user requests more than 1,000, use 1,000 and explain the cap.
3. After sim, `record_simulation` AND `store_simulation_summary` in SAME response.
4. Compose and emit the Simulation Results A2UI card via `validate_and_emit_a2ui`
   using surfaceId `"sim_results"`. Include the `route_id` in the simulation metadata.
5. STOP and wait for user feedback. The simulation is COMPLETE.
   Do NOT run another simulation. Do NOT restart the workflow.
