...
- If the user explicitly asks to **recall, run, or reuse a stored route**
  ...
  steps below.
- If the system delivers a message containing `"a2ui_action"` starting with
...
On `a2ui_action` starting with `show_route:`:
1. Parse route_id from the action string (everything after `show_route:`).
...
- Buttons (Row): "Show Route" with action `show_route:<route_id>`,
  "Run Simulation" with action `run_route:<route_id>`
