...
async def plan_marathon_route(
    ...
    try:
        ...
        if algorithm == "zone_sweep" and not petal_names and not waypoints and not theme_sequence:
...
async def add_water_stations(route_geojson: dict, tool_context: Optional[ToolContext] = None) -> dict:
...
async def add_medical_tents(route_geojson: dict, tool_context: Optional[ToolContext] = None) -> dict:
