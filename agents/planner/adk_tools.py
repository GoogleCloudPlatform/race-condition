...
_MAPS_TOOL_NAME_PREFIX = "maps"


def _normalize_maps_toolset(toolset):  # noqa: ANN001
    """Normalize an MCP toolset returned by AgentRegistry.get_mcp_toolset().

    Two adjustments:

    1. Strip ADC headers (Authorization, x-goog-user-project) so that the
       API key auth via header_provider is not overridden.
    2. Override the auto-derived ``tool_name_prefix``. AgentRegistry derives
       it from the MCP server's display name -- for ``mapstools.googleapis.com``
       that yields ``mapstools_googleapis_com``, which the LLM does not
       reliably call. We replace it with the short ``maps`` prefix that
       ``agents/planner/skills/mapping/SKILL.md`` teaches the model to use.
       The cache populated lazily by the prefix machinery
       (``_cached_prefixed_tools``) is invalidated so the next render uses
       the new prefix.
    """
    ...
    if hasattr(toolset, "tool_name_prefix"):
        toolset.tool_name_prefix = _MAPS_TOOL_NAME_PREFIX
    if hasattr(toolset, "_cached_prefixed_tools"):
        toolset._cached_prefixed_tools = None
...
def get_maps_tools() -> list:
    ...
    return [_normalize_maps_toolset(toolset)]
