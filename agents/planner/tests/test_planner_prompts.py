...
def test_planner_instruction_does_not_mention_simulator():
    ...
    instruction = PLANNER.build().lower()
...
def test_planner_with_eval_has_a2ui_instructions():
    ...
    assert "validate_and_emit_a2ui" in instruction, "planner_with_eval must reference validate_and_emit_a2ui"
    assert "A2UI" in instruction, "planner_with_eval must reference A2UI"
...
def test_planner_with_memory_inherits_a2ui_instructions():
    ...
    assert "validate_and_emit_a2ui" in instruction, "planner_with_memory must reference validate_and_emit_a2ui"
    assert "A2UI" in instruction, "planner_with_memory must reference A2UI"
...
class TestSerialPlanReportToolCalling:
    """Verify workflow keeps report/weather/landmarks on separate steps.

    The previous parallel-tool wording ("call X and Y in the SAME response")
    triggered Bug B in Gemini 3 Flash Preview: the post-tool continuation turn
    returned a thought-signature-only Part, ADK exited via is_final_response,
    and no terminal narrative reached chat. Reverted in topic/qa-regression-fixes
    after demo1_harness.py showed 7/20 vs 31/31 reliability.
    """

    def test_workflow_does_not_parallelize_report_with_maps_lookups(self):
        """report_marathon_route must not share its step with maps lookups."""
        workflow = PLANNER.sections["workflow"]
        # The instruction added by 5b332732 used the literal phrase
        # "in the SAME response" near `report_marathon_route`. The serial
        # workflow does not.
        assert "SAME response" not in workflow, "Workflow re-introduces the parallel-tool wording that caused Bug B"

    def test_workflow_report_after_route_generation(self):
        """report_marathon_route must come after plan_marathon_route (unchanged)."""
        workflow = PLANNER.sections["workflow"]
