...
def get_agent():
    ...
    return LlmAgent(
        ...
        generate_content_config=types.GenerateContentConfig(
            # max_output_tokens=8192 matches planner_with_eval's
            # empirically working config for flash. Without this, Gemini's
            # default cap combined with thinking_budget=1024 can starve
            # text emission on post-tool continuation turns (Bug B). See
            # docs/plans/2026-04-18-planner-max-output-tokens-design.md.
            max_output_tokens=8192,
            temperature=0.1,
            # gemini-3-flash-preview defaults to dynamic thinking and can
            # emit a thought_signature Part with text=None on terminal
            # turns -- leaving the chat empty (Demo 1 regression).
            # Pin a budget to externalize narrative output, matching
            # planner_with_eval (agents/planner_with_eval/agent.py:50).
