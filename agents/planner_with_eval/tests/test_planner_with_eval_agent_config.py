...
"""Regression: planner_with_eval LlmAgent must pin a thinking_budget.
...
def test_planner_with_eval_has_thinking_budget_set():
    """planner_with_eval MUST set thinking_config to externalize narrative.
    ...
    assert cfg is not None, "planner_with_eval must declare a generate_content_config"
    assert cfg.thinking_config is not None, (
        "planner_with_eval must set thinking_config; default behavior emits "
        "thought_signature without externalized text on gemini-3-flash-preview."
    )
    assert cfg.thinking_config.thinking_budget == 1024


def test_planner_with_eval_max_output_tokens_is_8192():
    """planner_with_eval MUST set max_output_tokens=8192."""
    from agents.planner_with_eval.agent import root_agent

    cfg = root_agent.generate_content_config
    assert cfg is not None, "planner_with_eval must declare a generate_content_config"
    assert cfg.max_output_tokens == 8192, (
        f"planner_with_eval must set max_output_tokens=8192. Currently: {cfg.max_output_tokens!r}"
    )
