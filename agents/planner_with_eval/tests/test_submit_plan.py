...
class TestExecuteReentrancyGuard:
    """Verify that submit_plan_to_simulator rejects duplicate execute calls.

    The guard is scoped by invocation_id: duplicate calls within the same
    invocation (LLM response) are blocked. A new invocation (new user
    message) resets the guard automatically.
    """
    ...
    async def test_first_execute_sets_invocation(self):
        ...
        assert result["status"] == "success"
        assert ctx.state.get("simulation_executed_invocation") == "inv-001"

    @pytest.mark.asyncio
    async def test_verify_action_not_blocked(self):
        """action='verify' must NOT be blocked even after a prior execute."""
        ctx = _make_tool_context(
            state={**self.ROUTE_STATE, "simulation_executed_invocation": "inv-001"},
        )
    ...
    @pytest.mark.asyncio
    async def test_new_invocation_allows_rerun(self):
        """A new invocation (different invocation_id) must allow re-execution."""
        ctx = _make_tool_context(
            state={
                **self.ROUTE_STATE,
                "simulation_executed_invocation": "inv-old",
                "simulator_session_id": "old-session",
            },
        )
        # The mock has invocation_id="inv-001" which differs from "inv-old"
        ...
        assert result["status"] == "ready"
        # Must generate a NEW session for the re-run
        assert ctx.state["simulator_session_id"] != "old-session"

    @pytest.mark.asyncio
    async def test_start_simulation_blocked_same_invocation(self):
        """start_simulation must also be blocked in the same invocation."""
        ctx = _make_tool_context(
            state={
                **self.ROUTE_STATE,
                "simulation_executed_invocation": "inv-001",
            },
        )
    ...
    @pytest.mark.asyncio
    async def test_full_rerun_cycle(self):
        """Full cycle: execute -> blocked -> new invocation -> execute succeeds."""
        ctx = _make_tool_context(state={**self.ROUTE_STATE})

        # First execute succeeds
        with patch("agents.utils.communication.call_agent", return_value="ack"):
            r1 = await submit_plan_to_simulator(
                action="execute",
                message="Run",
                tool_context=ctx,
            )
        ...
        # Second execute in same invocation is blocked (LLM looping)
        r2 = await submit_plan_to_simulator(
            action="execute",
            message="Run again",
            tool_context=ctx,
        )
        ...
        # start_simulation in same invocation is also blocked
        r3 = await start_simulation(
            action="execute",
            message="Run again",
            tool_context=ctx,
        )
        ...
        # User sends new message -> new invocation_id
        ctx.invocation_id = "inv-002"

        # start_simulation now succeeds (different invocation)
        r4 = await start_simulation(
            action="execute",
            message="Re-run",
            tool_context=ctx,
        )
        ...
        # execute also succeeds
        with patch("agents.utils.communication.call_agent", return_value="ack"):
            r5 = await submit_plan_to_simulator(
                action="execute",
                message="Run re-run",
                tool_context=ctx,
            )
