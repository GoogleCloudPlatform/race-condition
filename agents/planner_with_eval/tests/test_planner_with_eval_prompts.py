...
from agents.planner_with_eval.prompts import EXTENDED_SYSTEM_INSTRUCTION, PLANNER_WITH_EVAL
...
def test_planner_instruction_contains_new_tools():
    ...
    instruction = EXTENDED_SYSTEM_INSTRUCTION.lower()
...
def test_planner_with_eval_inherits_route_once_constraint():
    ...
    assert "EXACTLY ONCE" in EXTENDED_SYSTEM_INSTRUCTION
...
def test_planner_with_eval_mentions_single_pass_evaluator():
    ...
    instruction = EXTENDED_SYSTEM_INSTRUCTION
...
def test_prompts_include_button_instruction():
    ...
    assert "Button" in EXTENDED_SYSTEM_INSTRUCTION
    assert "run_simulation" in EXTENDED_SYSTEM_INSTRUCTION
    assert "Run Simulation" in EXTENDED_SYSTEM_INSTRUCTION


def test_prompts_include_a2ui_action_handler():
    """The prompts must include instructions for handling A2UI actions."""
    assert "a2ui_action" in EXTENDED_SYSTEM_INSTRUCTION
...
def test_planner_prompt_includes_quality_priorities():
    ...
    from agents.planner.prompts import PLANNER_INSTRUCTION

    assert "Plan Quality Priorities" in PLANNER_INSTRUCTION
    assert "Safety" in PLANNER_INSTRUCTION
    assert "Community" in PLANNER_INSTRUCTION
    assert "Intent Alignment" in PLANNER_INSTRUCTION
    assert "Logistics" in PLANNER_INSTRUCTION
    assert "Participant Experience" in PLANNER_INSTRUCTION
    assert "Financial" in PLANNER_INSTRUCTION
...
class TestEvalBuilderArchitecture:
    ...
    def test_build_matches_backward_compat_string(self):
        """PLANNER_WITH_EVAL.build() must equal EXTENDED_SYSTEM_INSTRUCTION."""
        assert PLANNER_WITH_EVAL.build() == EXTENDED_SYSTEM_INSTRUCTION
...
class TestWorkflowExecutionSplit:
    ...
    def test_full_prompt_still_contains_both_actions(self):
        ...
        prompt = EXTENDED_SYSTEM_INSTRUCTION
...
class TestSimResultsA2UISection:
    ...
    def test_sim_results_has_detail_rows(self):
        """Must include metric detail rows (Total distance, Participants, etc.)."""
        a2ui = PLANNER_WITH_EVAL.sections["a2ui"]
        assert "Total distance" in a2ui
        assert "Participants" in a2ui
        assert "Peak Hour Volume" in a2ui
...
class TestParallelToolCalling:
    ...
    def test_workflow_parallelizes_evaluate_and_verify(self):
        """evaluate_plan AND verify must be on the same workflow step."""
        workflow = PLANNER_WITH_EVAL.sections["workflow"]
        # Find the numbered step that mentions evaluate_plan
        lines = workflow.split("\n")
        eval_step_lines = [i for i, line in enumerate(lines) if "evaluate_plan" in line]
        assert eval_step_lines, "evaluate_plan must appear in workflow"
        # The same step (or continuation lines) must mention verify
        eval_idx = eval_step_lines[0]
        # Grab the step and its continuation lines (until next numbered step)
        step_lines = [lines[eval_idx]]
        ...
        assert "submit_plan_to_simulator" in step_text, (
            "evaluate_plan and submit_plan_to_simulator must be on the same step"
        )
...
class TestParallelExecution:
    ...
    def test_execution_parallelizes_start_and_submit(self):
        """start_simulation AND submit_plan_to_simulator in SAME response."""
        execution = PLANNER_WITH_EVAL.sections["execution"]
