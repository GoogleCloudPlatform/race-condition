...
class StartSimulationTool(BaseTool):
            ...
            parameters=types.Schema(
                ...
                properties={
                    "action": types.Schema(
                        ...
                        description="Either 'verify' or 'execute'.",
                    ),
                    "message": types.Schema(
                        type=types.Type.STRING,
                        description="A narrative summary of the plan for the simulator.",
                    ),
                    "simulation_config": types.Schema(
                        type=types.Type.OBJECT,
                        description="Optional simulation config dict.",
                        properties={
                            "duration_seconds": types.Schema(type=types.Type.INTEGER),
                            "tick_interval_seconds": types.Schema(type=types.Type.INTEGER),
                            "runner_count": types.Schema(type=types.Type.INTEGER),
                            "runner_type": types.Schema(type=types.Type.STRING),
                        },
                    ),
                    "runner_type": types.Schema(
                        type=types.Type.STRING,
                        description=(
                            "Optional runner type (e.g. 'runner', 'runner_gke', 'runner_cloudrun'). "
                            "Injected into simulation_config."
                        ),
                    ),
                },
    ...
    async def run_async(
        ...
        simulation_config = args.get("simulation_config") or {}
        runner_type = args.get("runner_type")
        ...
        if runner_type:
            ...
            simulation_config["runner_type"] = runner_type
...
class SubmitPlanToSimulatorTool(BaseTool):
            ...
            parameters=types.Schema(
                ...
                properties={
                    ...
                    "simulation_config": types.Schema(
                        type=types.Type.OBJECT,
                        description=(
                            "Optional config dict with keys: duration_seconds, "
    ...
    async def run_async(
        ...
        try:
            ...
            # Persist the simulator response to session state so downstream
            # memory tools (record_simulation, store_simulation_summary) can
            # read it without the LLM having to transcribe the JSON back
            # through a function-call argument.
            tool_context.state["simulation_result"] = response
