...
from agents.runner_autopilot.autopilot import (
    Phase,
    autopilot_callback,
    build_summary,
    detect_phase,
...
class TestHandleTick:
    ...
    def test_initializes_wave_parameters(self) -> None:
        """initialize_runner should set wave_number and start_delay_minutes."""
        state: dict = {}
        initialize_runner(state, "test-wave-session", runner_count=200)
        assert "wave_number" in state
        assert "start_delay_minutes" in state
        assert isinstance(state["wave_number"], int)
        assert state["start_delay_minutes"] >= 0.0

    def test_wave_assignment_favors_fast_runners(self) -> None:
        """Faster runners (lower target_finish) should get earlier waves."""
        # Use two sessions that produce very different target finishes.
        # We can't control the exact finish time, but with 200 runners
        # and 3 waves, the distribution should assign different waves
        # to runners with very different abilities.
        state_a: dict = {}
        state_b: dict = {}
        initialize_runner(state_a, "fast-runner-abc", runner_count=200)
        initialize_runner(state_b, "slow-runner-xyz", runner_count=200)
        # At minimum, both should have valid wave params
        assert 0 <= state_a["wave_number"] <= 2
        assert 0 <= state_b["wave_number"] <= 2
        assert state_a["start_delay_minutes"] >= 0.0
        assert state_b["start_delay_minutes"] >= 0.0
    ...
    def test_handle_tick_includes_inner_thought_arg(self) -> None:
        """Task I made inner_thought required on process_tick. The autopilot
        builds the call manually (no LLM) so it must include the arg even
        though it has nothing meaningful to say. Empty string preserves
        existing autopilot semantics."""
        state = {"velocity": 1.0, "finished": False, "collapsed": False}
        response = handle_tick(
            state,
            {"tick": 5, "max_ticks": 24, "minutes_per_tick": 15.0},
        )
        fc = _get_fc(response)
        args = fc.args
        assert args is not None, "autopilot tool call must have args"
        assert "inner_thought" in args, (
            f"autopilot must include inner_thought (required by process_tick); got args={dict(args)}"
        )
