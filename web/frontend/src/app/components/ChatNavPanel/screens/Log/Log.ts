...
export class LogComponent implements OnInit, OnDestroy {
  ...
  private speakerDict = {
    ...
    simulator: 'SIMULATOR',
    prepare_simulation: 'SIMULATOR',
    fire_start_gun: 'RUNNER',
  };
  ...
  ngOnInit(): void {
    this.sub = this.gateway.chat$.subscribe((msg: ChatMessage) => {
      if (msg.msgType === 'tool_end' && msg.toolName === 'prepare_simulation') {
        this.speakers.add(this.getSpeakerDisplayName('prepare_simulation'));
      } else if (msg.msgType === 'tool_end' && msg.toolName === 'fire_start_gun') {
        this.speakers.add(this.getSpeakerDisplayName('fire_start_gun'));
      } else {
        this.speakers.add(this.getSpeakerDisplayName(msg.speaker));
      }

      if (msg.msgType === 'run_start' && msg.speaker.includes('planner_with_memory')) {
        this.speakers.add('PLANNER');
      } else if (msg.msgType === 'run_start' && msg.speaker.includes('simulation_pipeline')) {
        this.speakers.add('SIMULATOR');
      } else if (msg.msgType === 'tool_end' && msg.toolName === 'fire_start_gun') {
        this.speakers.add('RUNNER');
      }

      if (
        msg.msgType === 'tool_end' &&
        msg.toolName === 'check_race_complete' &&
        msg.result.status === 'race_complete'
      ) {
        this.speakers.delete('RUNNER');
      }
      // else if  (msg.msgType === 'run_end' && msg.data.agent.includes('simulator')) {
      //   msg.
      // check_race_complete","result":{"status":"race_complete -> speakers delete runner
      // run_end","data":{"agent":"simulator-> speakers delete simulator
      // run_end planner with mem-> speakers delete planner
