...
import { demoFiveSpeaker, SECURE_AGENT_PROMPT } from '../../../../../constants';
...
/** A single step within an agent activity group (tool call, model response, etc.) */
interface ActivityStep {
  ...
  tokens?: number;
  result?: any;
  progress?: number; // 0–100, shown as a progress bar when set
}

/** Represents a combined agent invocation (run_start → run_end) shown as one expanding card */
interface ActivityGroup {
  ...
  finalText: string; // model_end text content to show as the main message
}
...
type DisplayItem =
  ...
  | { kind: 'tool_call'; msg: ChatMessage; done?: boolean; error?: boolean }
  | TickProgressItem;
...
@Component({
  ...
  template: `
    ...
    <simulation-panel
      ...
      [numberOfRaceAbandoners]="numberOfRaceAbandoners"
      ...
      (expand)="showSimPanel = false; isExpanded = true"
    ></simulation-panel>
    <div [hidden]="showSimPanel">
      <!-- existing chat content -->

      <div class="panel-wrapper" [class.thinking]="isAgentWorking">
        <div class="panel" [class.closed]="!isExpanded">
            ...
            <div class="panel__header__title">
              <div class="tab-bar">
                ...
                >
                  ...
                  <span *ngIf="activeTab === 'agent'">Agent</span>
                </button>
                <button
                  class="tab-btn"
                  [class.tab-btn--active]="activeTab === 'organizer'"
                ...
                >
                  ...
                  <span *ngIf="activeTab === 'organizer'">Organizer</span>
                </button>
                <button
                  class="tab-btn"
                ...
                >
                  ...
                  <span *ngIf="activeTab === 'log'">Log</span>
                </button>
              </div>
                ...
                <div class="settings-dropdown" *ngIf="settingsOpen">
                  <div class="settings-body">
                    ...
                    <div *ngIf="isSecurityDemo" class="settings-row settings-row--inline">
                      <div class="secure-toggle" (click)="setSecureMode(!secureMode)">
                        <button
                          [disabled]="secureMode"
                          class="secure-toggle__label"
                          [class.active]="!secureMode"
                        >
                          Secure Planner
                        </button>
                      </div>
                    ...
                    >
                      <div class="secure-toggle" (click)="setCloudAssistMode(!cloudAssistMode)">
                        <button
                          [disabled]="cloudAssistMode"
                          class="secure-toggle__label"
                          [class.active]="!cloudAssistMode"
                        >
                          Fix with Gemini Cloud Assist
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              <button
                class="expand-btn"
                (click)="isSimulationRunning ? (showSimPanel = true) : (isExpanded = !isExpanded)"
              >
          ...
          >
            <div class="chat-messages" #chatScroll (scroll)="onChatScroll()">
              <div *ngIf="displayItems.length === 0" class="empty">No messages yet</div>
              <ng-container
              ...
              >
                ...
                <ng-container *ngIf="item.kind === 'tool_call'">
                  <div class="tool-call" [class.expanded]="expandedToolCalls.has(i)">
                    <div class="header">
                      <div class="status" [class.error]="item.done && item.error">
                        <span class="material-icons tool-call-error" *ngIf="item.done && item.error"
                          >close</span
                        >
                        <span *ngIf="item.done && !item.error" class="material-icons tool-call-done"
                          >check</span
                        >
                      ...
                      <div class="top-content">
                        <div class="information">
                          <span class="tool-call-label">TOOL_CALL</span>
                ...
                <ng-container *ngIf="item.kind === 'message'">
                  ...
                  >
                    <div class="chat-msg-header">
                      <span class="error-icon" *ngIf="item.msg.msgType === 'tool_error'"
                        >error_outline</span
                      >
                      <h2 *ngIf="item.msg.msgType === 'tool_error'">Error message</h2>
                      <span class="chat-time">{{ item.msg.timestamp | date: 'h:mm a' }}</span>
                    </div>
                    <div
                      class="chat-text"
...
export class AgentScreenComponent implements OnInit, OnDestroy {
  ...
  private readonly onSimRunnerEvent = (e: Event): void => {
    this.ngZone.run(() => {
      const d = (e as CustomEvent).detail as {
        guid?: string;
        event?: string;
      };
      if (d.event === 'collapsed') {
        this.numberOfRaceAbandoners++;
      }
      // if (d.event === 'finished' && d.guid) {
      //   console.log(d.guid, 'finish from be');
  ...
  private readonly onHudUpdateSimRunnerForFinishers = (e: Event): void => {
    ...
    // console.log('FE Prog', d.guid, d.progress);

    if (d.progress && d.progress >= FINISH_UI_PROGRESS_T) {
      if (this.runnersFinishedAwaitingHud.has(d.guid)) return;
      else {
        this.runnersFinishedAwaitingHud.add(d.guid!);
    ...
    // if (d.status === 'did-not-finish') return;
    // const byStatus = d.status === 'finished';
    // const byProgress = d.progress !== undefined && d.progress >= FINISH_UI_PROGRESS_T;
    // console.log({
    //   status: d.status,
    //   progress: d.progress,
    //   vel: d.velocity,
    // });
    // if (!byStatus && !byProgress) return;
    // if (!this.runnersFinishedAwaitingHud.has(d.guid)) return;
    // console.log(d.guid, 'finish from fe?');
    // this.ngZone.run(() => {
    //   if (!this.runnersFinishedAwaitingHud.has(d.guid!)) return;
    //   this.runnersFinishedAwaitingHud.delete(d.guid!);
    //   this.numberOfFinishers++;
    //   this.cdr.markForCheck();
    // });
  };
  ...
  async setCloudAssistMode(value: boolean) {
    if (value) {
      ...
      await this.onInitAgent('planner_with_memory');

      this.currentAgent = {
        agentType: 'planner_with_memory',
        sessionId: this.agents['planner_with_memory'] as string,
      };
    ...
    effect(async () => {
      ...
      if (activeDemo.placeholderRoutes) {
        ...
        window.dispatchEvent(
          new CustomEvent('gateway:routeGeojson', { detail: { geojson: routeJson.route_data } }),
        );
      }
  ...
  ngOnInit(): void {
    ...
    // window.addEventListener('sim:raceStarted', () => {
    //   this.ngZone.run(() => this.handleRaceStarted());
    // });

    // window.addEventListener('sim:tickUpdate', (e) => {
    //   this.ngZone.run(() => this.handleTickUpdate((e as CustomEvent).detail));
    // });
  ...
  private handleTickUpdate(d: Record<string, any>): void {
    this.averageDistance = d['avg_distance'] ?? 0;
    this.numberOfActiveRunners = d['runners_reporting'] ?? 0;
    this.averageWater = d['avg_water'] ?? 0;
  ...
  get canSend(): boolean {
    return !!this.agents[this.currentAgent!.agentType] && Object.keys(this.agents).length > 0;
  }
  ...
  private processMessageForDisplay(msg: ChatMessage): void {
    ...
    switch (msg.msgType) {
      ...
      case AgentMessageType.TOOL_END:
        if (msg.result && (msg.result.beginRendering || msg.result.surfaceUpdate)) {
        ...
        if (msg.toolName === 'fire_start_gun') {
          ...
          //   simulationRealTimeMinutes: this.simulationRealTimeMinutes,
          //   averageDistance: this.averageDistance,
          //   numberOfFinishers: this.numberOfFinishers,
          //   numberOfRaceAbandoners: this.numberOfRaceAbandoners,
          //   averagePace: this.averagePace,
          //   averageWater: this.averageWater,
          // });
        ...
        if (msg.toolName === 'get_local_laws_and_regulations') {
          const tempPayload =
            '```json\n' +
            JSON.stringify({ message: { regulations: msg.result.regulations } }, null, 2) +
            '\n```';
          this.finishToolCall({
            ...msg,
            text: tempPayload,
      ...
      case AgentMessageType.TEXT:
        ...
        return;
      case AgentMessageType.TICK:
        if (msg.result) {
          let resultData = msg.result;
          if (typeof msg.result === 'string') {
            try {
              resultData = JSON.parse(msg.result);
            } catch (e) {
              console.error('Failed to parse tick result:', e);
            }
          }

          if (resultData && resultData.tick != null && resultData.maxTicks != null) {
            const existing = this.tickItems.get(msg.guid);

            if (existing) {
              if (msg.guid !== this.agents['tick_agent']) return;
              existing.tick = resultData.tick + 1;
              existing.maxTicks = resultData.maxTicks;
              // Mutate in-place so the same array reference update triggers re-render
              this.displayItems = [...this.displayItems];

              // allow for new simulations
              if (resultData.tick + 1 === resultData.maxTicks) {
                this.agents['tick_agent'] = null;
              }
            } else {
              // new tick start
              if (resultData.tick === 1) {
                this.agents['tick_agent'] = msg.guid;

                const tickItem: TickProgressItem = {
                  kind: 'tick_progress',
                  guid: msg.guid,
                  tick: resultData.tick,
                  maxTicks: resultData.maxTicks,
                  label: msg.toolName || 'Simulation running',
                  done: false,
                };

                this.tickItems.set(msg.guid, tickItem);
                this.displayItems = [...this.displayItems, tickItem];
              }
            }
          }
        }
