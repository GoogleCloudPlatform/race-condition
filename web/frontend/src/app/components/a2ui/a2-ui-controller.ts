  ...
  template: `
    ...
    <ng-template #renderer let-node="node">
      <ng-container *ngIf="node">
        <div
          [class]="
            'a2ui-' + getType(node) + (isExpandableSummaryCard(node) ? ' a2ui-Card-expandable' : '')
          "
        ...
        >
          <ng-container [ngSwitch]="getType(node)">
            ...
            <ng-container *ngSwitchCase="'Button'">
              <button
                *ngIf="!(isOpenCardButton(node) && alwaysExpandExpandableCards)"
              ...
              >
                <span *ngIf="isOpenCardButton(node)" class="material-icons unfold">{{
...
export class A2uiControllerComponent implements OnChanges {
  ...
  actionRegistry = {
    ...
    show_plan: (payload?: any) => {
      const routes = {
        'Strip Classic': routes01StripClassic,
        'Entertainment Circuit': routes02EntertainmentCircuit,
        'East Side Explorer': routes03DiagonalTraverse,
        'Grand Loop': routes04NorthSouthExpress,
      };
      const route = routes[payload as keyof typeof routes];
      console.log('show_plan', route);

      window.dispatchEvent(
        new CustomEvent('gateway:routeGeojson', {
          detail: { geojson: route.route_data },
        }),
      );
    },
    organizer_show_scorecard: () => {
      window.dispatchEvent(new CustomEvent('organizer:showScorecard'));
    },
    show_route_geojson: (payload?: { geojson?: unknown }) => {
      window.dispatchEvent(
        new CustomEvent('gateway:routeGeojson', {
          detail: { geojson: payload?.geojson },
        }),
      );
    },
  };
  ...
  isOpenCardButton(node: any): boolean {
    const actionData = this.getData(node)?.action;
    return this.resolveValue(actionData?.name) === 'open_card';
  }

  /** Whether the expandable card containing this open_card button is expanded. */
  ...
  handleButtonClick(event: any, node: any): void {
    ...
    if (actionName === 'organizer_show_scorecard' || actionName === 'show_route_geojson') {
      // @ts-expect-error
  ...
  isExpandableSummaryCard(node: any): boolean {
    ...
    return first != null && this.getId(first) === 'header';
  }
  ...
  // toggleOption(node: any, value: string): void {
  //   const id = this.getId(node);
  //   const data = this.getData(node);
  //   const max = data.maxAllowedSelections ?? Infinity;
  //   const initial: string[] = this.resolveValue(data.selections) || [];
  //   const current = this.selectionsMap.has(id)
  //     ? [...this.selectionsMap.get(id)!]
  //     : [...initial];
  ...
  //     current.push(value);
  //   }
  //   this.selectionsMap.set(id, current);
  // }
}
