...
enum TOOLS {
  ...
  GET_LAWS = 'get_local_laws_and_regulations',
  ...
  handleMessage(messageEvent: MessageEvent) {
    ...
    try {
      ...
      if (
        ...
        const inferredType = origin.id || origin.type || 'unknown';
      ...
      // Decode payload bytes once — shared across all cases
      ...
      const label = session?.displayName || agentType;
      const speaker = `${label} (${sessionId.substring(0, 6)})`;
      ...
      if (this.subscribedSimulationId && wrapper.simulationId !== this.subscribedSimulationId) {
        return;
      }
        ...
        case 'json': {
          switch (event) {
            case 'run_start':
              ...
              return;
            case 'run_end':
              if (session && this.userSpawnedAgents.has(sessionId)) {
                ...
                case TOOLS.SPAWN_RUNNERS:
                  ...
                  if (Array.isArray(sessionIds)) {
                    for (const runnerId of sessionIds) {
                      ...
                      if (!this._knownRunnerSessions.has(runnerId)) {
