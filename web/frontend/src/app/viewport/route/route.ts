...
import { getStartGoal, getInfoIcon, getWaterZone, getMedicalZone, getCrowdZone } from './icons';
...
export function removeRunnerRoute(ctx: Context): void {
  ...
  ctx.zones = [];
  ctx.zonesMaterialArray = [];
  // Do not clear routeCanvasTexture / maskMap1 / ground emissive here — they are shared with the
  // planning-route overlay. Clearing them before async work in initRoute / initRunnerRoute left
  // maskMap1 null for many frames (flash). Full teardown uses removeRoute().
  ctx.routeDraw = null;
}
...
// Todo: refactor in to single texture mask, keep masks for lightmap, traffic que and traffic mask out
