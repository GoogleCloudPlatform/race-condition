  ...
  template: `
    ...
    <div #runnerLabel class="runner-container">
      <div class="inner" *ngIf="!runnersHaveThoughts; else haveThoughts">
        <p class="emoji"></p>
        <div class="stats">
          <h3 class="runner-label"></h3>
          <p class="runner-data">Position #123</p>
  ...
  styles: [
    `
      ...
      .runner-container {
        ...
        border-radius: 60px;
        border: 1px solid var(--Neutral-White---10, rgba(255, 255, 255, 0.1));
        background: linear-gradient(180deg, rgba(35, 40, 42, 0.3) 0%, rgba(24, 24, 24, 0.3) 100%);
        ...
        .inner {
          ...
          .stats {
            ...
            color: var(--neutral-white-60, rgba(255, 255, 255, 0.6));
            /* Body/H5-Medium */
      ...
      .icon-tooltip {
        ...
        border-radius: 40px;
        border: 2.5px solid var(--neutral-white-5, rgba(255, 255, 255, 0.05));
        background: linear-gradient(180deg, rgba(35, 40, 42, 0.1) 0%, rgba(24, 24, 24, 0.1) 100%);
...
export class ViewportComponent implements OnInit, OnDestroy {
  ...
  private readonly wheelHandler = (e: WheelEvent): void => {
    if (this.ctx.cameraFollow?.autoRotate) {
      e.preventDefault();
      followZoom(e.deltaY * 2);
    }
  };
  ...
  // Matches default race grid: 6 ticks × 60 sim-min/tick = 6h sim, 10s wall between ticks → 1 sim-h per 10s wall.
  ...
  private tickRunnerLabel(): void {
    ...
    el.querySelector('.emoji')!.textContent =
      place === 1 ? '🥇' : place === 2 ? '🥈' : place === 3 ? '🥉' : `${runner?.getEmoji()}`;

    el.querySelector('.runner-label')!.textContent = this.runnersHaveThoughts
      ? runnerThought
      : `Runner ${activeGuid!.split('-')[1].slice(0, 5)}`;
    el.querySelector('.runner-data')!.textContent = this.runnersHaveThoughts
      ? `ID: ${activeGuid!.split('-')[1].slice(0, 5)} #${place}`
      : `#${place} Energy level: ${water}%`;

    if (!this.runnersHaveThoughts) {
      el.querySelector('.runner-pace')!.textContent = `Pace: ${pace} min/mile`;
    }
  }

  private debugTickRunnerLabel(): void {
    const el = this.runnerLabelRef.nativeElement;
    ...
    const world = this.currentSpline.points[0].clone();
    world.project(this.ctx.camera);
    const x = (world.x * 0.5 + 0.5) * window.innerWidth;
    const y = (-world.y * 0.5 + 0.5) * window.innerHeight;
    el.style.display = 'flex';
    ...
    // el.querySelector('.runner-label')!.textContent =
    // el.querySelector('.runner-data')!.textContent =
  }
  ...
  private animate(): void {
    ...
    if (this.ctx.runnerRoute && this.ctx.debug) {
      // test/debug runners (tweakpane only)
      ...
      for (let i = 0; i < this._simRunnerMeshes.length; i++) {
        ...
        if (time >= 1 || runner.status === 'finished') {
          if (!mesh.userData['_finished']) {
            ...
            if (this._followLeaderActive && this.followGuid === guid) {
              this._followLeaderActive = false;
              this.followGuid = null;
              simLog.log('CAMERA', guid, 'leader finished, starting post-finish sequence');
        ...
        if (leaderMesh) {
          ...
          if (leaderGuid !== this.followGuid) {
            ...
            if (this.ctx.cameraFollow) {
              ...
              followMesh(this.ctx, leaderMesh, new THREE.Vector3(-1000, 1000, 1000), this._autoPanning);
            }
    ...
    //const temp = new THREE.Vector3().copy(this.ctx.camera.position).sub(this.ctx.controls.target);
    //console.log(this.ctx.controls.target, temp);
  ...
  private onImportPath = async (e: Event): Promise<void> => {
    ...
    const camOffset = new THREE.Vector3(-radius * 0.8, radius * 1.2, radius * 0.6);
    ...
    await this.initRoute(spline, rawWaterStationPoints, rawMedicalTentsPoints, rawCrowdZonesPoints);
  ...
  private onSimSpawnRunners = async (e: Event): Promise<void> => {
    ...
    const offset = new THREE.Vector3(1000, 1000, 1000);
    if (this.currentSpline) {
  ...
  private onFocusSimRunner = (e: Event): void => {
    ...
    if (this.ctx.cameraFollow) {
      ...
      followMesh(this.ctx, mesh, new THREE.Vector3(1000, 1000, 1000), this._autoPanning);
    }
  };
  ...
  public panCameraTo(targetPosition: THREE.Vector3, cameraOffset: THREE.Vector3): Promise<void> {
    return panCameraTo(this.ctx, targetPosition, cameraOffset);
  }
  ...
  public followMesh(
    ...
    followMesh(this.ctx, mesh, offset, autoRotate, offsetTransition);
  }

  // Stops following a mesh
  public stopFollowMesh(): void {
    if (this.ctx.controls.autoRotate) {
  ...
  public followRandomRunner(): void {
    const infos = this.runnerManager
      .getRunnerInfos()
      .filter((i) => i.status === 'running' && i.guid !== this.followGuid);
    ...
    if (this.ctx.cameraFollow) {
      ...
      followMesh(this.ctx, runner.getMesh(), new THREE.Vector3(-1000, 1000, 1000), this._autoPanning);
    }
  }
  ...
  private startPostFinishSequence(): void {
    ...
    followMesh(this.ctx, this.ctx.currentRouteGoal, offset, false, undefined, 1);
  ...
  public async debugStartZone(): Promise<void> {
    ...
    triggerStartZoneAnimation(
      ...
      1.5,
      async () => {
        console.log("startZone complete");
