...
import { EXTERNAL_THOUGHTS, roadSpline1, roadSpline2, roadSpline3 } from '../../constants';
...
export class ViewportComponent implements OnInit, OnDestroy {
  ...
  cycleThoughts = () => {
    this.currentExternalThought =
      EXTERNAL_THOUGHTS[Math.floor(Math.random() * EXTERNAL_THOUGHTS.length)];
  };
  ...
  private readonly onGiveRunnersThoughts = (): void => {
    ...
    this.cycleThroughThoughtsInterval = setInterval(this.cycleThoughts, 2000);
  ...
  private tickRunnerLabel(): void {
    ...
    const runnerThought = this.currentExternalThought;
