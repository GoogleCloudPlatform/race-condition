...
export function followMesh(
  ...
  ctx.controls.enabled = autoRotate;

  if (autoRotate) {
    _occInitialized = false;
    _occTargetPolar = FOLLOW_DEFAULT_POLAR;
    _occClearTimer = 0;
    ctx.controls.autoRotate = false;
    ctx.controls.enabled = true;
    ...
    ctx.camera.position.set(
      target.x + Math.sin(azimuth) * Math.sin(FOLLOW_DEFAULT_POLAR) * dist,
      target.y + Math.cos(FOLLOW_DEFAULT_POLAR) * dist,
      target.z + Math.cos(azimuth) * Math.sin(FOLLOW_DEFAULT_POLAR) * dist,
    );
    ctx.controls.update();
  }
}
...
export function tickCameraFollow(ctx: Context, delta: number): void {
  ...
  const smoothness = ctx.cameraFollow.positionSmoothness ?? 2;
  const targetSmooth = 1 - Math.exp(-smoothness * delta);

  if (ctx.cameraFollow.autoRotate) {
    ctx.controls.target.lerp(targetPos, targetSmooth);

    if (!_occInitialized) {
      const toCamera = new THREE.Vector3().subVectors(ctx.camera.position, ctx.controls.target);
      _occDist = toCamera.length();
      _occAzimuth = Math.atan2(toCamera.x, toCamera.z);
      _occPolar = Math.acos(Math.max(-1, Math.min(1, toCamera.y / _occDist)));
      _occTargetPolar = _occPolar;
      _occSmoothedPos.copy(ctx.camera.position);
    ...
    _occAzimuth += OCC_AUTO_ROTATE_SPEED * delta;

    const origin = ctx.controls.target;

    if (ctx.occlusionMeshes.length > 0) {
      const step = (OCC_AUTO_ROTATE_SPEED * OCC_LOOKAHEAD) / 4;
      let blocked = false;
      for (let i = 0; i <= 4; i++) {
        const az = _occAzimuth + step * i;
        if (_isBlocked(ctx, origin, az, _occTargetPolar, _occDist)) {
          blocked = true;
          break;
        }
      }
      if (!blocked) {
        const conservativePolar = Math.min(_occTargetPolar + 0.1, FOLLOW_MAX_POLAR);
        for (let i = 0; i <= 4; i++) {
          const az = _occAzimuth + step * i;
          if (_isBlocked(ctx, origin, az, conservativePolar, _occDist)) {
            blocked = true;
            break;
          }
        }
      ...
      if (blocked) {
        _occClearTimer = 0;
        _occTargetPolar = Math.max(FOLLOW_MIN_POLAR, _occTargetPolar - delta * OCC_RISE_SPEED);
      } else {
        _occClearTimer += delta;
        if (_occClearTimer > OCC_CLEAR_DELAY && _occTargetPolar < FOLLOW_DEFAULT_POLAR) {
          const lowerPolar = Math.min(
            _occTargetPolar + delta * OCC_FALL_SPEED,
            FOLLOW_DEFAULT_POLAR,
          );
          let wouldBlock = false;
          for (let i = 0; i <= 4; i++) {
            const az = _occAzimuth + step * i;
            if (_isBlocked(ctx, origin, az, lowerPolar, _occDist)) {
              wouldBlock = true;
              break;
            }
          }
          if (!wouldBlock) {
            _occTargetPolar = lowerPolar;
          }
        }
      }
    }

    _occPolar += (_occTargetPolar - _occPolar) * (1 - Math.exp(-OCC_POLAR_LERP * delta));

    _posFromSpherical(origin, _occAzimuth, _occPolar, _occDist, _occIdealPos);

    const posLerp = 1 - Math.exp(-OCC_POS_LERP * delta);
    _occSmoothedPos.lerp(_occIdealPos, posLerp);

    ctx.camera.position.copy(_occSmoothedPos);
    ctx.camera.lookAt(origin);
    return;
  }
