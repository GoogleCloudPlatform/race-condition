...
variable "public_uis_unauthenticated" {
  description = "Bind allUsers as roles/run.invoker on the public-facing web UIs (frontend, admin, dash, tester). True by default for the OSS public demo; set to false to gate them behind IAP / per-user OIDC."
  type        = bool
  default     = true
}
