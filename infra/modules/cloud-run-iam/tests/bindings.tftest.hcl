...
#   - Public UIs (frontend, admin, dash, tester): allUsers bindings
#     gated by public_uis_unauthenticated.
...
run "public_uis_present_by_default" {
  command = plan

  assert {
    condition     = length(google_cloud_run_v2_service_iam_member.frontend_public) == 1
    error_message = "frontend allUsers binding must be present when public_uis_unauthenticated=true (default)"
  }
  ...
  assert {
    condition     = length(google_cloud_run_v2_service_iam_member.admin_public) == 1
    error_message = "admin allUsers binding must be present when public_uis_unauthenticated=true (default)"
  }

  assert {
    condition     = google_cloud_run_v2_service_iam_member.admin_public[0].member == "allUsers"
    error_message = "admin public binding member must be allUsers"
  }

  assert {
    condition     = length(google_cloud_run_v2_service_iam_member.dash_public) == 1
    error_message = "dash allUsers binding must be present when public_uis_unauthenticated=true (default)"
  }

  assert {
    condition     = google_cloud_run_v2_service_iam_member.dash_public[0].member == "allUsers"
    error_message = "dash public binding member must be allUsers"
  }

  assert {
    condition     = length(google_cloud_run_v2_service_iam_member.tester_public) == 1
    error_message = "tester allUsers binding must be present when public_uis_unauthenticated=true (default)"
  }

  assert {
    condition     = google_cloud_run_v2_service_iam_member.tester_public[0].member == "allUsers"
    error_message = "tester public binding member must be allUsers"
  }
}

run "public_uis_absent_when_disabled" {
  command = plan

  variables {
    public_uis_unauthenticated = false
  }

  assert {
    condition     = length(google_cloud_run_v2_service_iam_member.frontend_public) == 0
    error_message = "frontend allUsers binding must be absent when public_uis_unauthenticated=false"
  }

  assert {
    condition     = length(google_cloud_run_v2_service_iam_member.admin_public) == 0
    error_message = "admin allUsers binding must be absent when public_uis_unauthenticated=false"
  }

  assert {
    condition     = length(google_cloud_run_v2_service_iam_member.dash_public) == 0
    error_message = "dash allUsers binding must be absent when public_uis_unauthenticated=false"
  }

  assert {
    condition     = length(google_cloud_run_v2_service_iam_member.tester_public) == 0
    error_message = "tester allUsers binding must be absent when public_uis_unauthenticated=false"
  }
}
