...
[![Open in Cloud Shell](assets/cloud_shell_button.svg)](https://shell.cloud.google.com/cloudshell/editor?cloudshell_git_repo=https://github.com/GoogleCloudPlatform/race-condition.git&cloudshell_tutorial=tutorial.md&cloudshell_open_in_editor=scripts/deploy.sh)
...
Services default to scale-to-zero (`min_instances=0`) and `max_instances=1`, so you only pay for compute when actively using the application. The fixed infrastructure cost (Memorystore Redis, Cloud SQL, Cloud NAT) is ~$91/month regardless of usage. Each simulation run costs ~$3-4 in Gemini API calls. Use the `runner_autopilot` variant (deterministic, zero LLM calls) to develop and test without API costs.

The deploy entry point (`scripts/deploy.sh` and `make deploy`) shows this same breakdown and asks for confirmation before provisioning anything. After confirmation, it pre-flights your project (enables the APIs Cloud Build needs and grants the Cloud Build default SA the IAM roles required to run Terraform; idempotent), then submits the build. Tear down with `cd infra && terraform destroy`. To keep services warm and avoid cold starts, raise `min_instances` for the relevant services in `infra/terraform.tfvars` via the `service_sizing` map and re-apply.
