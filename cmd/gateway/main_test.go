...
func TestOrchestrationPush(t *testing.T) {
	...
	defer rc.Close()
	sb := hub.NewSwitchboard(rc, "test-gw", h, catalog)
...
func TestHealthCheck_RedisOffline(t *testing.T) {
	...
	defer rdb.Close()
	sb := hub.NewSwitchboard(rdb, "test-gw", h, nil)
	mr.Close()
...
func TestBatchSpawn_DefaultMaxRunners(t *testing.T) {
	// Unset MAX_RUNNERS — default should be 250.
	...
	h := hub.NewHub()
	mockSb := &MockSwitchboard{Done: make(chan bool, 251)}
	catalog := agent.NewCatalog([]string{agentServer.URL})
	router := setupRouter(h, mockSb, catalog, session.NewInMemorySessionService(), "test-gw", nil, nil)
	...
	// Should succeed with capped count (300 -> 250).
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200 (capped at default 250), got %d: %s", w.Code, w.Body.String())
	}
}
...
func TestMaxRunners_InvalidFallsBackToDefault(t *testing.T) {
	...
	var resp map[string]interface{}
	require.NoError(t, json.Unmarshal(w.Body.Bytes(), &resp))
	if int(resp["max_runners"].(float64)) != 250 {
		t.Errorf("expected default max_runners=250, got %v", resp["max_runners"])
	}
}
...
func TestMaxRunners_ZeroFallsBackToDefault(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal(w.Body.Bytes(), &resp))
	if int(resp["max_runners"].(float64)) != 250 {
		t.Errorf("expected default max_runners=250 for zero input, got %v", resp["max_runners"])
	}
}
