...
// oidcAudience is the OIDC audience used by middleware.AttachOIDCToken
// when proxying requests to the gateway. Empty disables OIDC attachment.
// Resolved by main() via auth.ResolveAudience(gatewayURL) so dev/IAP
// and OSS/Cloud-Run-IAM modes work without per-callsite branching.
...
func main() {
	...
	// Audience for OIDC tokens attached to gateway-bound proxy requests.
	// IAP_CLIENT_ID wins (dev/prod IAP-fronted gateway); otherwise the
	// internal/public gateway URL is used (OSS Cloud Run IAM mode where
