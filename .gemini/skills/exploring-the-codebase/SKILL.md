...
### Runner Agents (`agents/runner/`, `agents/runner_autopilot/`)

Individual runner agents making per-tick decisions. Two variants:
- `runner/` -- LLM-powered (Gemini, Ollama, or vLLM). Contains all shared
  running mechanics (hydration, pacing, wave computation).
- `runner_autopilot/` -- Deterministic (zero LLM calls, constant pacing).
  Extends the base runner via `get_base_agent()`, overriding only the
  decision-making callback.
