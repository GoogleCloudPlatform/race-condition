...
> Generated from `gen_proto/gateway/gateway.proto` and the agent source.
> Hand-edit cautiously; large changes should be regenerated rather than patched.
...
| 4 | `session_id`  | `string`          | Session routing identifier                                                 |
...
| :----------- | :------------- | :------------------------------- |
| `load_skill` | `skill_name`   | `{"skill_name": "brainstorming"}`|
...
#### Parameterized Action Names

Some actions encode identifiers directly in the `actionName` string because only
the action name survives the `a2ui_action` pipeline (no payload data is
forwarded to the agent). The agent parses the identifier from the action name.

| Action Pattern              | Agent                  | Behavior                                          |
| :-------------------------- | :--------------------- | :------------------------------------------------ |
| `run_simulation`            | `planner_with_eval`+   | Trigger simulation execution                      |
| `organizer_show_scorecard`  | Frontend (local)       | Expand score card in organizer (no agent round-trip) |
| `show_route:<route_id>`     | `planner_with_memory`  | Visualize a stored route on the frontend viewport  |
...
**Gateway Event**: The tool result is emitted as a JSON event wrapped in the
standard `tool_end` envelope (see Section 3, JSON Events):
```json
{
  "tool_name": "set_financial_modeling_mode",
  "result": {
    "status": "success",
...
```
The frontend detects `tool_name === "set_financial_modeling_mode"` and reads the
mode from `result.financial_modeling_mode`. This triggers a UI reset (clear chat,
reset prompt placeholder) to prepare for the security demo flow.
...
the starting line (distance=0; starting water varies per runner in the
88-100% range based on ability and randomized prep). After publishing, advances
...
| `route_list`   | `get_planned_routes_data`  | Route cards with name, score, distance, and two buttons: "Open Score Card" (`organizer_show_scorecard`) and "Show Route" (`show_route:<route_id>`) |

#### Show Route Workflow

When the frontend sends a `show_route:<route_id>` action (see Section 5,
Parameterized Action Names), the agent:

1. Parses the `route_id` from the action string
2. Calls `get_route(route_id=..., activate_route=True)` to hydrate session state
3. Calls `report_marathon_route` to emit the route GeoJSON via the standard
   `tool_end` event pipeline
4. The frontend receives the GeoJSON in a `tool_end` event and renders it on
   the 3D viewport

This is a **view-only** action — the agent does not start a simulation.

##### Frontend Integration Guide

To support this action, the frontend must register `show_route` in the
`a2-ui-controller.ts` action registry. The handler should send the full action
name (e.g., `show_route:seed-0001-strip-classic-a1b2c3d4`) back to the agent
via the `a2ui_action` protobuf pathway (same pattern as `run_simulation`). The
agent will call `report_marathon_route`, which emits route GeoJSON through the
standard `tool_end` event. The frontend already handles route visualization
via the `gateway:routeGeojson` custom event — the route data arrives in the
`tool_end` JSON payload under the `route_geojson` key.

Example `handleButtonClick` dispatch (the `actionName` variable is resolved from
the button's `action.name` field earlier in the handler):

```typescript
// In the actionRegistry, alongside run_simulation:
show_route: () => {
  // actionName is e.g. "show_route:seed-0001-strip-classic-a1b2c3d4"
  // Send the full action name back to the agent via a2ui_action protobuf
  this.sendA2UIAction(this.agentScreen.currentAgent!.sessionId, actionName);
},
```
...
- **Directory**: `agents/runner_autopilot/`
...
`agents/runner_shared/` and are re-exported by both the autopilot and the
...
re-export from `agents/runner_shared/`:
