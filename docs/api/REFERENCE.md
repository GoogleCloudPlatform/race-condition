...
| `run_route:<route_id>`      | `planner_with_memory`  | Run simulation for a stored route by UUID          |
...
| `route_list`   | `get_planned_routes_data`  | Route cards with name, score, distance, and two buttons: "Show Route" (`show_route:<route_id>`) and "Run Simulation" (`run_route:<route_id>`) |
