...
A2UI (Agent-to-User Interface) is a schema-driven protocol designed to enable AI agents to deliver rich, interactive UI components to frontends via standard message parts.

The `n26-devkey-simulation-code` project strictly follows the **a2ui.org:standard_catalog_0_8_0** specification.

## Core Philosophy

- **Capitalized Primitives**: All component types MUST use capitalized names (e.g., `Video`, `Card`, `Column`). Legacy `a2ui-` prefixes are strictly forbidden.
- **Direct Properties**: Properties are mapped directly to the component object. The legacy `props` wrapper is deprecated.
...
- **LLM-Driven Layout**: Layout is achieved through the composition of atomic primitives, minimizing the need for custom one-off components.
...
The `tester` UI utilizes a **Recursive Rendering Engine** in `web/tester/src/a2ui/index.ts`. All 18 primitives are mapped in the `REGISTRY`.
