...
PROJECT_ID = args.project_id or os.environ.get("PROJECT_ID", "your-gcp-project-id")
...
GCS_FILE_LOCATION = args.gcs_file_location or os.environ.get("GCS_FILE_LOCATION", "gs://your-bucket/laws_and_regulations")
CITY_NAME = args.city

# AlloyDB Connection properties
alloydb_ip = args.alloydb_ip or os.environ.get(
    "ALLOYDB_IP", "127.0.0.1"
)  # Uses IP of AlloyDB proxy which is locally bound, or Dev DB
alloydb_port = os.environ.get("ALLOYDB_PORT", "8104" if is_local else "5432")
...
jdbc_url = (
    f"jdbc:postgresql://{alloydb_ip}:{alloydb_port}/postgres?sslmode=disable&options=-c%20search_path={alloydb_schema}"
)
...
def process_document(gcs_uri: str):
    ...
    if not PROCESSOR_ID:
        with open("/tmp/spark_worker.log", "a") as f:
            f.write("Warning: PROCESSOR_ID is missing on worker.\n")
        return []
    ...
    try:
        ...
        for i in range(0, total_pages, MAX_PAGES_PER_CALL):
            ...
            with open("/tmp/spark_worker.log", "a") as f:
                f.write(f"Created chunk content of {len(chunk_content)} bytes for {gcs_uri}\n")
            ...
            try:
                ...
                if not text and hasattr(document, "document_layout"):
                    try:
                        ...
                        with open("/tmp/spark_worker.log", "a") as f:
                            f.write(f"Layout extraction error: {e}\n")

                with open("/tmp/spark_worker.log", "a") as f:
                    f.write(
                        f"Extracted {len(text) if text else 0} chars, {len(document.pages) if hasattr(document, 'pages') and document.pages else 0} pages. Doc preview: {str(document)[:100]}\n"
                    )
                if not text:
                    continue
        ...
        with open("/tmp/spark_worker.log", "a") as f:
            f.write(f"Returning {len(chunks)} chunks for {gcs_uri}\n")
        return chunks
    except Exception as e:
        with open("/tmp/spark_worker.log", "a") as f:
...
def main():
    with open("/tmp/spark_main.log", "w") as f:
        f.write("main() started\n")

    if is_local:
        with open("/tmp/spark_main.log", "a") as f:
            f.write("LOCAL_DEV is true\n")
        ...
        session = Session()
        spark = (
            DataprocSparkSession.builder.config("spark.dataproc.engine", "lightningEngine")
    ...
    with open("/tmp/spark_main.log", "a") as f:
        f.write("Created uri_df\n")

    # 2 & 3. Read them using Google Document AI and chunk them semantically
    chunk_schema = ArrayType(
        StructType([StructField("document_uri", StringType(), False), StructField("chunk_text", StringType(), False)])
    )

    process_udf = udf(process_document, chunk_schema)
    chunked_df = (
        uri_df.withColumn("chunks", process_udf(col("gcs_uri")))
    ...
    final_df = (
        chunked_df.withColumn("source_file", F.element_at(F.split("document_uri", "/"), -1))
    ...
    with open("/tmp/spark_main.log", "a") as f:
        f.write("Created final_df\n")

    if is_local:
        with open("/tmp/spark_main.log", "a") as f:
            f.write("Connecting to local JDBC\n")
        ...
        if count > 0:
            final_df.write.format("jdbc").option("url", jdbc_url).option("dbtable", target_table).option(
                "user", alloydb_user
            ).option("password", alloydb_pass).option("driver", "org.postgresql.Driver").mode("append").save()
    else:
        ...
        pool = sqlalchemy.create_engine(
            "postgresql+pg8000://", creator=getconn, connect_args={"options": f"-c search_path={alloydb_schema}"}
        )
