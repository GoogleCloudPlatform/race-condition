...
These tests verify the pure functions and structural contracts of the
OSS deployment script. They do NOT call gcloud or terraform; all
external commands are mocked.

The deploy module is loaded from the file alongside this test so the
tests work both in the monorepo (where this file lives at
``scripts/oss/templates/scripts/deploy/deploy_test.py``) AND in a
synced OSS repo (where it lives at ``scripts/deploy/deploy_test.py``
beside ``scripts/deploy/deploy.py``). Using ``import scripts.deploy.deploy``
in the monorepo would silently bind to the internal deploy.py instead
of the OSS template, so we use importlib to load the sibling file
unambiguously.
"""
...
class TestBuildResourceUrl:
    """The Cloud Build orchestrator captures the agent's A2A resource URL with
    `... | tail -1`. deploy_agent_engine MUST return that exact string so
    main()'s `print(url)` puts a clean URL on the final stdout line. This
    helper isolates the URL-shape so the contract is unit-testable without
    booting the full vertexai stack."""
...
class TestServicesRegistry:
    """SERVICES dict must contain only the 5 OSS Agent Engine agents and
    declare every field deploy_agent_engine() consumes. Right-sizing per
    docs/plans/2026-04-16-oss-right-sizing-plan.md is also asserted here so
    cost defaults can't silently regress."""
    ...
    def test_all_ae_agents_have_at_least_2gi(self):
        """All AE agents need >=2Gi memory baseline (Phase 7 verification).

        Empirical: 1Gi worked intermittently in early builds but failed
        reproducibly under repeat deploys with the generic Vertex error
        "failed to start and cannot serve traffic". The ADK runtime
        adds enough memory overhead on top of the user agent module
        that 1Gi has no margin -- workers OOM during startup imports
        before the AE health probe ever gets a 200.

        2Gi is the verified safe baseline for all AE agents in OSS,
        independent of whether they declare extra_packages. Pinning
        this prevents future config drift (someone adds a new agent
        and copies an old 1Gi config).
        """
        for name, cfg in deploy.SERVICES.items():
...
class TestDetermineDeployMode:
    """The deploy mode resolution must:

    1. Honor force_create unconditionally.
    2. Fast path: if {SERVICE}_INTERNAL_URL is set and the engine still
       exists on GCP, return ('update', resource_name).  This is how
       dev/prod has always worked.
    3. Fallback (OSS path): if the env var is absent, list AEs in the
       project and find one with displayName == service_name.  Required
       for OSS Cloud Build, where no env var source exists; without this
       every build orphans a fresh AE per agent.
...
class TestFindEngineByDisplayName:
    ...
    - Return None on an empty AE list (first deploy).
    - Return None when no engine matches the requested display name.
    - When multiple engines share the display name, pick the most-
      recently-updated one (so subsequent deploys converge on the active
      copy, not a random orphan).
    """
...
class TestListReasoningEngines:
    """The discovery seam must:

    - Return [] silently for genuinely-empty projects (NotFound is OK).
    - Re-raise PermissionDenied / quota / network errors so deploy.py exits
      non-zero in Cloud Build, instead of silently falling through to
      ('create', None) and re-orphaning the AE the OSS user just lost.

    The previous bare ``except Exception`` regenerated the exact orphan-AE
    failure mode that gotcha #40's discovery fallback was added to prevent
    -- caught in code review on commit 5265f6c.
    """
...
class TestCli:
    """CLI flags --only and --print-url for per-agent AE deploy from Cloud Build.

    The cloudbuild-bootstrap.yaml orchestrator runs five parallel steps that each
    deploy ONE Agent Engine agent and capture its resource URL via shell:
      AE_URL=$(uv run python scripts/deploy/deploy.py --only planner --print-url | tail -1)
    These tests lock in the contract that --only filters to a single agent and
    --print-url emits ONLY the resource URL on the final stdout line.
    """
