...
Single load-bearing entry point used by ``cloudbuild-bootstrap.yaml``:

    python scripts/deploy/deploy.py --only <agent> --print-url

Five parallel Cloud Build steps invoke this once each (planner, simulator,
planner_with_eval, simulator_with_failure, planner_with_memory). Each step
captures the printed A2A resource URL with ``... | tail -1`` and writes it
to ``/workspace/ae_urls/<agent>.url`` for downstream collection into the
gateway's ``AGENT_URLS`` env var.

Cloud Run services + IAM bindings are owned by the Terraform
``cloud-run-services`` and ``cloud-run-iam`` modules (gated by
``var.enable_services``). Image builds run as parallel ``build-{service}``
steps in cloudbuild-bootstrap.yaml. DB schema + seed + embedding backfill
run as ``null_resource``s in Terraform's ``cloud-sql-postgres`` module.
None of those concerns live in this script.
"""
...
def _read_terraform_outputs() -> dict:
    ...
    1. ``/workspace/tf_outputs.json`` -- written by the ``read-tf-outputs``
       Cloud Build step. Required for the ``python:3.13-slim`` AE deploy
       steps, which lack a ``terraform`` binary. Without this, the
       subprocess call below raises ``FileNotFoundError``, the catch-all
       silently falls back to env vars (which aren't set), and Vertex AI
       rejects ``agent_engines.create()`` with "Required field is not set"
       on empty REDIS_ADDR / PUBSUB_TOPIC_ID env vars.
    2. ``terraform -chdir=infra output -json`` via subprocess -- local dev
       path where the developer has terraform on PATH.
    3. Environment variables -- last-resort fallback (current behaviour).
    """
...
# resource_limits per docs/plans/2026-04-16-oss-right-sizing-plan.md:
#   simulators run light (1Gi); planners need more memory for prompts (2Gi).
#   max_instances=1 caps concurrent agent runtime cost.
SERVICES = {
    "simulator": {
        ...
        # 2Gi (not 1Gi) is the OSS-verified baseline for ALL AE agents.
        # 1Gi worked intermittently in early Phase 7 builds but failed
        # reproducibly in builds #18 + #19 with the same generic
        # "failed to start and cannot serve traffic" error -- worker
        # OOM during ADK runtime startup, no margin for the runtime's
        # own overhead. Matches planner's 2Gi.
    ...
    "simulator_with_failure": {
        ...
        # 2Gi (not 1Gi) because extra_packages doubles the module-import
        # footprint at AE startup (loads simulator's module + its own
        # across ~4 uvicorn workers). 1Gi causes silent OOM-kills before
        # the AE health probe gets a 200 -- surfaces as "failed to start
        # and cannot serve traffic". Matches planner_with_eval and
        # planner_with_memory, which also use extra_packages + 2Gi.
        "resource_limits": {"memory": "2Gi", "cpu": "1"},
        "max_instances": 1,
    },
    "planner_with_memory": {
...
# Mockable seams: keep all vertexai SDK calls behind module-level
# helpers so deploy_test.py can patch them without monkeying with the
# SDK itself.
#
# Location for AE list/get/create is implicit from the
# `vertexai.init(project=..., location=...)` call in `deploy_agent_engine`,
# not a per-helper parameter. The SDK doesn't accept a location filter
# on `agent_engines.list()`.


def _is_not_found(exc: Exception) -> bool:
    """True if the exception is a 'resource genuinely does not exist' signal.

    Discriminates the legitimate "first deploy / no AE yet" signal (return
    safe sentinel) from auth/quota/network errors (must propagate so the
    Cloud Build step exits non-zero instead of silently re-orphaning the
    AE that gotcha #40's discovery fallback was supposed to find).

    Uses isinstance against ``google.api_core.exceptions.NotFound`` when
    that module is importable (production path), AND a class-name fallback
    so tests can use a stand-in NotFound class without depending on the
    google api core exception hierarchy.
    """
    try:
...
def _lookup_engine_by_resource_name(resource_name: str) -> bool:
    """Return True iff an Agent Engine with this resource_name still exists.

    Wraps ``agent_engines.get()``. Returns False on NotFound (the env var
    points at an engine that has since been deleted -- expected, fall
    through to discovery). Re-raises permission/quota/network errors so
    they fail the deploy loudly rather than silently degrading.
    """
    try:
...
def _list_reasoning_engines() -> list:
    """Return a list of Agent Engines in the configured project/location.

    Returned objects expose ``display_name``, ``resource_name``, and
    ``update_time`` attributes (as the vertexai SDK does).

    Returns ``[]`` only when the project genuinely has no AEs (NotFound).
    Permission/quota/network errors are re-raised so the Cloud Build step
    exits non-zero -- silently returning ``[]`` would regenerate the exact
    orphan-AE failure mode that gotcha #40 was added to prevent.
    """
    try:
        from vertexai import agent_engines
...
def _find_engine_by_display_name(service_name: str) -> str | None:
    """Discover an existing Agent Engine by displayName.

    Used as a fallback in OSS deploys, where ``{SERVICE}_INTERNAL_URL``
    is never set (no env source). Without this fallback, every Cloud
    Build run calls ``agent_engines.create()`` and orphans a fresh AE
    per agent, accumulating duplicates over time.

    When multiple engines share the display name (typical after several
    pre-fix deploys), returns the most-recently-updated one so future
    deploys converge on the active copy rather than picking a random
    orphan. Engines without an ``update_time`` attribute are treated as
    "older than the dawn of time" so a real candidate always wins.

    Returns the resource_name string or None if no match is found.
    """
...
def _determine_deploy_mode(
    ...
    *,
    ...
    Resolution order:
      1. force_create                               -> ("create", None)
      2. {SERVICE}_INTERNAL_URL points at a live AE -> ("update", env-pointed)
      3. Discovery by display_name finds an AE     -> ("update", discovered)
      4. Otherwise                                  -> ("create", None)

    The env-var fast path (step 2) is the dev/prod contract and is
    untouched. The discovery fallback (step 3) is what makes the OSS
    Cloud Build "deploy is idempotent across re-runs" property hold,
    where the env var is naturally absent.
    """
    if force_create:
        return "create", None
...
def deploy_agent_engine(service_name: str, cfg: dict, *, tf: dict, force_create: bool = False) -> str:
    ...
    Supports create-or-update: if an existing engine is found via
    ``{SERVICE}_INTERNAL_URL``, the engine is updated in-place.

    Returns the A2A resource URL string. Callers (specifically ``main()``
    when ``--print-url`` is set) emit this as the final stdout line so the
    cloudbuild-bootstrap.yaml orchestrator can capture it via
    ``... | tail -1``.
    """
    import vertexai
    ...
    # Return the A2A resource URL string. The CLI's --print-url path emits
    # this as the final stdout line, captured by the cloudbuild-bootstrap.yaml
    # orchestrator via `... | tail -1`.
    return a2a_url
