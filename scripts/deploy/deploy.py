...
def _read_terraform_outputs() -> dict:
    ...
    _terraform_outputs_cache = {
        ...
        # project_number is the canonical numeric id baked into Cloud Run
        # service URLs (https://<service>-<project_number>.<region>.run.app).
        # We use it to derive the gateway URL deterministically inside the
        # cloud-build python:3.13-slim step (which has no gcloud), so AE
        # engines get GATEWAY_URL set on first deploy without any post-deploy
        # patching.
        "project_number": outputs.get("project_number", os.getenv("PROJECT_NUMBER", "")),
        ...
        # `pubsub_topic` is a legacy alias kept for backward compat with older
        # tf states; it returns the SAME value as `orchestration_topic`. Do not
        # use it for telemetry -- agents that publish telemetry to the
        # orchestration topic produce JSON the gateway's Wrapper-proto parser
        # rejects with HTTP 400, flooding Pub/Sub retries indefinitely.
        ...
        "telemetry_topic": outputs.get(
            "telemetry_topic",
            os.getenv("PUBSUB_TOPIC_ID", "agent-telemetry"),
        ),
...
def _construct_gateway_url(project_number: str, region: str) -> str:
    """Construct the gateway's Cloud Run service URL deterministically.

    Cloud Run service URLs follow the pattern
    ``https://<service>-<project_number>.<region>.run.app``. We use this
    instead of ``gcloud run services describe`` because the ``deploy-ae-*``
    cloudbuild steps run in the slim ``python:3.13-slim`` image which has
    no gcloud binary AND the gateway Cloud Run service hasn't been
    provisioned yet at that point in the pipeline (it's created later by
    ``tf-apply-services``). The deterministic URL works because Cloud Run
    URL composition is purely a function of project number + region +
    service name -- no chicken-and-egg with actual service existence.

    Returns an empty string if project_number is missing (caller will
    handle the absent-env-var case).
    """
    if not project_number:
        return ""
    return f"https://gateway-{project_number}.{region}.run.app"
...
# waiting on Gemini API responses, not local compute. max_instances=1
# caps the per-agent fan-out so a runaway demo can't accidentally rack
# up a Vertex bill (one autopilot replica is sized to handle 100
# simulated runners). min_instances stays at the AE default (0) so
# engines scale to zero between simulations.
SERVICES = {
    "simulator": {
...
# Labels applied to every Agent Engine deployed by this script. Mirrors the
# tf provider's default_labels (projects/oss/main.tf) so AE engines and
# tf-managed resources end up with the same label set for cost attribution
# and demo-asset cleanup. Override the JSON value via the AE_LABELS env var.
_DEFAULT_AE_LABELS = {"dev-tutorial": "race-condition"}


def _resolve_ae_labels() -> dict:
    raw = os.getenv("AE_LABELS", "")
    if not raw:
        return dict(_DEFAULT_AE_LABELS)
    try:
        parsed = json.loads(raw)
        if isinstance(parsed, dict):
            return {str(k): str(v) for k, v in parsed.items()}
    except (json.JSONDecodeError, TypeError) as e:
        print(f"  AE_LABELS env var was not valid JSON ({e}); using defaults")
    return dict(_DEFAULT_AE_LABELS)


def _apply_labels_to_engine(resource_name: str, location: str, labels: dict) -> None:
    """PATCH the AE engine's labels via REST.

    The vertexai.agent_engines Python SDK doesn't expose a `labels`
    parameter on create()/update(), so we patch them post-deploy. Failure
    to apply labels is non-fatal: the engine itself works either way, and
    labels are only used for cost attribution / cleanup.
    """
    if not labels:
        return
    ...
    url = f"{base}/v1beta1/{resource_name}?updateMask=labels"
    try:
        ...
        if 200 <= resp.status_code < 300:
            ...
            print(f"   WARNING: failed to apply labels (HTTP {resp.status_code}): {resp.text[:200]}")
    except Exception as e:  # noqa: BLE001
        print(f"   WARNING: failed to apply labels: {e}")
...
def deploy_agent_engine(service_name: str, cfg: dict, *, tf: dict, force_create: bool = False) -> str:
    ...
    # Construct gateway URL deterministically from project_number + region.
    # See _construct_gateway_url() for the rationale (the slim Cloud Build
    # image has no gcloud and the gateway Cloud Run service doesn't exist
    # yet at deploy-ae time -- both ruled out a runtime gcloud lookup).
    gateway_url = _construct_gateway_url(tf.get("project_number", ""), tf["region"])
    ae_env_vars = {
        ...
        # PUBSUB_TOPIC_ID drives the DashLog plugin's telemetry publisher
        # (agents/utils/plugins.py RedisDashLogPlugin). It MUST be a separate
        # topic from ORCHESTRATION_TOPIC_ID -- the orchestration topic has a
        # push subscription to the gateway that strictly parses messages as
    ...
    # Apply labels post-create (the SDK doesn't accept them on create/update).
    # Mirrors the tf provider's default_labels so AE engines and Cloud Run
    # services end up with the same label set.
    _apply_labels_to_engine(resource_name, ae_location, _resolve_ae_labels())
