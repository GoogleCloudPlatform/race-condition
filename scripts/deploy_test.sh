...
# Unit tests for scripts/deploy.sh.
#
# Pattern matches scripts/e2e/e2e_dispatch_test.sh: pure bash, manual
# PASS/FAIL counter, single-file runner. Source deploy.sh with
# DEPLOY_SH_NO_MAIN=1 so the helper functions load without firing
# the interactive prompts.
#
# Usage:
#   bash scripts/oss/templates/scripts/deploy_test.sh   (from monorepo root)
#   bash scripts/deploy_test.sh                          (from OSS repo root)
...
# --- TF variables.tf path resolution ---------------------------------------
# Smart default-detection so tests work in BOTH layouts:
#   * Monorepo (or worktree): walk up to backend/, then sibling code-infra/.
#     Looks for both .worktrees/topic/oss-deploy/projects/oss/variables.tf
#     and projects/oss/variables.tf.
#   * OSS synced: scripts/deploy_test.sh -> ../infra/variables.tf
# Override with DEPLOY_SH_TF_VARS_FILE if the auto-detect ever drifts.
