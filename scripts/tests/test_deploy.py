...
class TestServiceURLConstruction:
    ...
    def test_gateway_url_uses_per_service_domain(self):
        ...
        env_vars = deploy.build_env_vars("gateway", env_name="dev", domain="sim.example.test")
        gateway_url = next(v for v in env_vars if v.startswith("GATEWAY_URL="))
        assert gateway_url == "GATEWAY_URL=https://gateway.dev.sim.example.test"
    ...
    def test_runner_autopilot_url_uses_per_service_domain(self):
        ...
        env_vars = deploy.build_env_vars("runner_autopilot", env_name="dev", domain="sim.example.test")
        ra_url = next(v for v in env_vars if v.startswith("RUNNER_AUTOPILOT_URL="))
        assert ra_url == "RUNNER_AUTOPILOT_URL=https://runner-autopilot.dev.sim.example.test"
    ...
    def test_prod_a_domain_pattern(self):
        ...
        env_vars = deploy.build_env_vars("admin", env_name="prod-a", domain="sim.example.test")
        admin_url = next(v for v in env_vars if v.startswith("ADMIN_URL="))
        assert admin_url == "ADMIN_URL=https://admin.prod-a.sim.example.test"
...
class TestNewEnvVars:
    ...
    def test_cors_allowed_origins_present(self):
        ...
        assert "tester.dev.sim.example.test" in origins
        assert "admin.dev.sim.example.test" in origins
...
class TestAgentEngineConfig:
    ...
    def test_self_contained_agents_have_no_extra_packages(self):
        ...
        simulator_with_failure is intentionally NOT in this list -- it
        re-exports simulator's skill set and bundles agents/simulator as an
        extra_package (see commit 8043283e).
        """
        for name in ("simulator", "planner"):
