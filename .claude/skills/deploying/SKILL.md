...
Review the plan, confirm. This drops every GCP resource Terraform created
(VPC, Redis, Cloud SQL/AlloyDB, NAT, the lot).

### Step 2: clean up anything not managed by Terraform

If anyone created resources outside Terraform (one-off Cloud Run deploys,
extra Agent Engine agents), they don't get cleaned up automatically:
